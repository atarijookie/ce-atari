...
FloppyImage *FloppyImageFactory::getImage(const char *fileName)
{
    ...
    if(strcasecmp(ext, "zip") == 0) {       // if it's a ZIP file
        logFdd(LOG_DEBUG, "FloppyImageFactory -- file %s is a ZIP file, will search for supported image inside", fileName);
        ...
        if(!foundValidImage) {              // image not found in ZIP file, fail
            logFdd(LOG_DEBUG, "FloppyImageFactory -- ZIP file %s doesn't contain supported image inside", fileName);
            return NULL;
        }

        logFdd(LOG_DEBUG, "FloppyImageFactory -- ZIP file %s contains image: %s", fileName, fileNameInZip.c_str());
    ...
    if(strcasecmp(ext, "msa") == 0) {   // msa image?
        logFdd(LOG_DEBUG, "FloppyImageFactory -- using MSA image on %s", fileName);
        img = new FloppyImageMsa();

    } else if(strcasecmp(ext, "st") == 0) {    // st image?
        logFdd(LOG_DEBUG, "FloppyImageFactory -- using ST image on %s", fileName);
        img = new FloppyImageSt();
    } else {
        logFdd(LOG_DEBUG, "FloppyImageFactory -- Image file %s type %s not supported", fileName, ext);
    }
