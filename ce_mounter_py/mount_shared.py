...
def get_shared_mount_command(nfs_not_samba):
    ...
    app_log.debug(f"get_shared_mount_command: mount_cmd_file is: {mount_cmd_file}")
    ...
    app_log.debug(f"get_shared_mount_command: will use this mount_cmd: {mount_cmd}")
    return mount_cmd


def mount_shared():
    """ this function checks for shared drive settings, mounts drive if needed, but also triggers reload in core """
    mounted = mount_shared_internal()

    if mounted:     # if something was mounted / symlinked, trigger reload in core
        trigger_reload_translated()
...
def mount_shared_internal():
    ...
    if not shared_enabled:                  # if shared drive not enabled, don't do rest, possibly umount
        ...
        app_log.debug(f"mount_shared: SHARED_ENABLED={shared_enabled}, not mounting")
    ...
    app_log.info(f"mount_shared: shared drive is_mounted: {is_mounted}")
    ...
    good = False
    try:
        app_log.info(f'mount_shared: cmd: {cmd}')
        status = os.system(cmd)         # try mounting
        good = status == 0              # good if status is 0
        app_log.info(f'mount_shared: good={good}')
