...
def get_not_mounted_devices(devs, mounts_in):
    ...
    """
    app_log.debug(f"get_not_mounted_devices: devs: {devs}, mounts_in: {mounts_in}")
...
def mount_device_to_mnt(device):
    ...
    # some error?
    if status != 0:
        app_log.warning(f'mount of {device} failed')
        return None

    # on success
    app_log.info(f'mount of {device} succeeded')
    return mount_dir
...
def mount_not_mounted_devices(root_devs, part_devs, mounts):
    ...
    app_log.info(f'not mounted: {root_devs_not_mounted}')
    ...
    for root_dev in root_devs_not_mounted:
        ...
        app_log.debug(f"for root device {root_dev} found partition devices {devices}")
...
def symlink_not_linked(mounted_all):
    ...
    app_log.debug(f'mounted_all       : {mounted_all}')
    app_log.debug(f'sources_existing  : {sources_existing}')
    app_log.debug(f'sources_not_linked: {sources_not_linked}')
    ...
    if len_letters < len_not_linked:
        app_log.warning((f'symlink_not_linked: not enough letters to symlink all not linked mounts. '
    ...
    for i in range(len_can):            # symlink all we can
        ...
        app_log.debug(f"symlink_not_linked: {source} -> {path}")

        try:
            good = symlink_if_needed(source, path)
            ...
            if not os.path.exists(path):    # check if created symlink exists and it's not broken
                app_log.warning(f"symlink_not_linked: symlink {source} -> {path} created but broken!")
                unlink_without_fail(path)   # delete the broken symlink
        except Exception as ex:
            app_log.warning(f"symlink_not_linked: failed to symlink {source} -> {path}: {str(ex)}")
...
def find_and_mount_translated(root_devs, part_devs):
    ...
    got_something = symlink_not_linked(mounted_all)
