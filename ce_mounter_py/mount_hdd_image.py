...
from shared import letter_shared, get_usb_drive_letters, get_symlink_path_for_letter, \
...
def resolve_hddimage_path(path_image: str):
    ...
    if os.path.exists(path_image) and os.path.isfile(path_image):
        app_log.info(f"resolve_hddimage_path: HDD image path {path_image} exists, using as is")
        return path_image
    ...
    if path_image.startswith("shared"):                         # should be looking for image only on shared drive?
        ...
        app_log.info(f"resolve_hddimage_path: will look for {path_image} on shared drive: {letter_shared()}")
        ...
        app_log.info(f"resolve_hddimage_path: will look for {path_image} on USB drives: {drive_letters}")
        ...
        app_log.info(f"resolve_hddimage_path: will look for {path_image} on ALL drives")
    ...
    for letter in drive_letters:
        ...
        for path in paths:          # go through all the matching patterns
            if os.path.exists(path) and os.path.isfile(path):   # path exists and it's a file? good, return it
                app_log.info(f"resolve_hddimage_path: resolved {path_image} to {path}")
                return path

    # if came here, nothing found, return None
    app_log.info(f"resolve_hddimage_path: couldn't resolve {path_image} any valid path")
    return None
...
def get_hddimage_path():
    ...
    app_log.debug(f"resolve_hddimage_path: HDDIMAGE {hdd_image}")

    if not hdd_image:                       # no hdd image in settings? just quit
        app_log.debug(f"resolve_hddimage_path: empty, not returning")
        return None
    ...
    if not hdd_image or not os.path.exists(hdd_image):  # if te file doesn't exist, return None
        app_log.debug(f"resolve_hddimage_path: file {hdd_image} doesn't exist")
        text_to_file("FAIL", os.getenv('FILE_HDDIMAGE_RESOLVED'))        # store nothing to resolved hdd image file
        return None

    app_log.debug(f"resolve_hddimage_path: file {hdd_image} exist, will use it")
...
def mount_hdd_image():
    ...
    if not hdd_image_path:                  # don't have path? just quit
        app_log.info(f"mount_hdd_image: no valid image path, not symlinking")
        return

    app_log.info(f"mount_hdd_image: will symlink {hdd_image_path} if needed now")
