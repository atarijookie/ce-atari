...
extern WORD streamTableOffset;              // the stream table starts at this offset, because first 5 words are empty (ATN + sizes + other)
extern WORD streamStartOffset;

void moveReadIndexToNextSector(void)
{
    inIndexGet = streamStartOffset;             // set get index to stream start in case we fail to find next sector index

    if(streamed.sector >= 1 && streamed.sector < STREAM_TABLE_ITEMS) {          // if streamed sector seems to be valid (will fit in stream table)
        WORD *pStreamTable = (WORD *) (readTrackData + streamStartOffset);      // get pointer to stream table
