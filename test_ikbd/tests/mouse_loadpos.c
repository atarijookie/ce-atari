...
int setAndQuery(const char *subTestName, const uint8_t *setCmd, int setCmdLen, const uint8_t *queryCmd, int queryCmdLen, const uint8_t *expectedResponse, int expectedResponseLength)
{
    uint8_t response[8];
...
int testCorner(const char *subTestName, const char *message, int upLeftNotBtmRight)
{
    ...
    uint8_t response[6];
    while(1) {
        ASSERT_SUCCESS( ikbd_puts((const uint8_t []) {0x0d},   1), subTestName )   // send QUERY cmd
    ...
    const uint8_t leftUp[4]    = {0, 0, 0, 0};
    const uint8_t rightDown[4] = {3, 0, 3, 0};
    const uint8_t *coords      = upLeftNotBtmRight ? leftUp : rightDown;           // choose with what it should be compared
...
int testClick(const char *subTestName, const char *message, int upLeftNotBtmRight)
{
    ...
    uint16_t maskUp     = upLeftNotBtmRight ? ABSMOUSEBTN_LEFTUP : ABSMOUSEBTN_RIGHTUP;
    uint16_t maskDown   = upLeftNotBtmRight ? ABSMOUSEBTN_LEFTDN : ABSMOUSEBTN_RIGHTDN;
...
uint8_t test_mouse_loadpos_run()
{
    int res;
    //------------------
    // set mouse to ABS mode
    res = setAndQuery("E1", (const uint8_t []) {0x09,3,0,3,0}, 5,              // set ABS mouse with max [0x300, 0x300]
                            (const uint8_t []) {0x89},         1,              // inquiry mode
                            (const uint8_t []) {0xf6, 9, 3,0, 3,0, 0,0}, 8);   // should report ABS, with max [0x300, 0x300]
    if(!res) return FALSE;

    //------------------
    // set mouse to [0,0], read back
    res = setAndQuery("E2", (const uint8_t []) {0x0e,0,0,0,0,0},    6,         // load mouse pos [0,0]
                            (const uint8_t []) {0x0d},              1,         // interrogate mouse
                            (const uint8_t []) {0xf7, 0, 0,0, 0,0}, 6);        // test the mouse pos [0,0]
    if(!res) return FALSE;

    //------------------
    // set mouse to [0x300,0x300], read back
    res = setAndQuery("E3", (const uint8_t []) {0x0e,0, 3,0, 3,0},  6,         // load mouse pos [0x300,0x300]
                            (const uint8_t []) {0x0d},              1,         // interrogate mouse
                            (const uint8_t []) {0xf7, 0, 3,0, 3,0}, 6);        // test the mouse pos [0x300,0x300]
    if(!res) return FALSE;
