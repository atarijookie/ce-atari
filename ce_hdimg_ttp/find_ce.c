...
BYTE ce_identify(BYTE id, BYTE hddIf)
{
    ...
    (*hdIf.cmd)(1, cmd, 6, (BYTE *)buffer, 1);                         // issue the identify command and check the result 
    
    if(!hdIf.success || hdIf.statusByte != OK) {                // if failed, return FALSE 
        return FALSE;
    }
    
    if(strncmp((char *) buffer, "CosmosEx config console", 23) != 0) { // the identity string doesn't match? 
        return FALSE;
    }
...
BYTE cs_inquiry(BYTE id, BYTE hddIf)
{
    ...
    (*hdIf.cmd)(1, cmd, 6, (BYTE *)buffer, 1);	                        // issue the inquiry command and check the result 
    
    if(!hdIf.success || hdIf.statusByte != OK) {                // if failed, return FALSE 
        return FALSE;
    }

    if(strncmp(((char *) buffer) + 16, "CosmoSolo", 9) != 0) { // the inquiry string doesn't match? fail
        return FALSE;
    }
