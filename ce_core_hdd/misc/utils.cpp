...
#include "../hdd/translated/translatedhelper.h"
#include "../hdd/translated/gemdos.h"
#include "debug.h"
#include "version.h"
#include "settings.h"
#include "global.h"
...
bool Utils::copyFile(std::string &src, std::string &dst)
{
    ...
    if(!from) {
        logHdd(LOG_ERROR, "Utils::copyFile - failed to open source file %s", src.c_str());
        return false;
    }
    ...
    if(!to) {
        ...
        logHdd(LOG_ERROR, "Utils::copyFile - failed to open destination file %s", dst.c_str());
        return false;
    }
...
void Utils::setTimezoneVariable_inProfileScript(void)
{
    ...
    logHdd(LOG_DEBUG, "Utils::setTimezoneVariable_inProfileScript() -- creating timezone setting script like this: %s\n", tzString);
...
void Utils::setTimezoneVariable_inThisContext(void)
{
    ...
    logHdd(LOG_DEBUG, "Utils::setTimezoneVariable_inThisContext() -- setting TZ variable to: %s\n", utcOfsset);
...
bool Utils::unZIPfloppyImageAndReturnFirstImage(const char *inZipFilePath, std::string &outImageFilePath)
{
    ...
    if(dir == NULL) {                               // not found?
        logHdd(LOG_DEBUG, "Utils::unZIPfloppyImageAndReturnFirstImage -- opendir() failed");
        return false;
    }
    ...
    if(!found) {                                    // not found? return with a fail
        logHdd(LOG_DEBUG, "Utils::unZIPfloppyImageAndReturnFirstImage -- couldn't find an image inside of %s", inZipFilePath);
        return false;
    }
...
std::string Utils::dotEnvValue(std::string key, const char* defValue, bool logOnError)
{
    ...
    try {
        ...
        if(logOnError) {
            logHdd(LOG_DEBUG, "Utils::dotEnvValue - no value for key '%s' !", key.c_str());
        }
    }
    ...
    if(envVar) {    // some real env var was found with this name?
        ...
        if(logOnError) {
            logHdd(LOG_DEBUG, "Utils::dotEnvValue - ...but found env var '%s' with value '%s'", key.c_str(), retValueFromEnv.c_str());
        }
...
void Utils::getDefaultValueFromVarName(std::string& varName, std::string& defValue, const std::string& delim)
{
    ...
    //logHdd(LOG_DEBUG, "Utils::getDefaultValueFromVarName - varName '%s'", varName.c_str());
    ...
    if(varDef != std::string::npos) {           // if this variable name has also default value specified
        ...
        //logHdd(LOG_DEBUG, "Utils::getDefaultValueFromVarName - varName with default: '%s', varName '%s', defValue: '%s'", varName.c_str(), newVarName.c_str(), defValue.c_str());
...
int Utils::dotEnvSubstituteVars(void)
{
    ...
    logHdd(LOG_DEBUG, "Utils::dotEnvSubstituteVars starting");
    ...
    while (it != dotEnv.end())                          // go through all map values
    {
        ...
        if(varStart != std::string::npos) {                 // start tag was found?
            ...
            //logHdd(LOG_DEBUG, "Utils::dotEnvSubstituteVars - varName '%s'", varName.c_str());

            if(varDef != std::string::npos) {           // if this variable name has also default value specified
                std::string newVarName = varName.substr(0, varDef); // get just var name without default value
                defValue = varName.substr(varDef + 2);              // get just the default value
                //logHdd(LOG_DEBUG, "Utils::dotEnvSubstituteVars - varName with default: '%s', varName '%s', defValue: '%s'", varName.c_str(), newVarName.c_str(), defValue.c_str());
                varName = newVarName;                               // use the new var name
            }

            std::string varValue = Utils::dotEnvValue(varName, defValue.c_str());   // get variable value with possible default value
            //logHdd(LOG_DEBUG, "Utils::dotEnvSubstituteVars - for var '%s' found value '%s'", varName.c_str(), varValue.c_str());

            value.replace(varStart, varEnd - varStart + 1, varValue);   // replace variable in original value
            //logHdd(LOG_DEBUG, "Utils::dotEnvSubstituteVars - value after replacing var: '%s'", value.c_str());
...
bool Utils::loadDotEnvFrom(const char* path)
{
    ...
    if(!f) {
        logHdd(LOG_ERROR, "Utils::loadDotEnv - failed to open file %s", path);
        return false;
    }
    ...
    while(true) {                           // go through file line by line
        ...
        //logHdd(LOG_DEBUG, "Utils::loadDotEnv - found %s -> %s", key.c_str(), value.c_str());

        dotEnv[key] = value;        // store to map
    }

    fclose(f);
    logHdd(LOG_INFO, "Utils::loadDotEnv - loaded file %s", path);
...
void Utils::sendToMounter(const std::string& jsonString)
{
	...
	if (sockFd < 0) {   // if failed to create socket
	    logHdd(LOG_ERROR, "sendToMounter: failed to create socket - errno: %d", errno);
	    return;
	}
    ...
    if(res < 0) {       // if failed to send
	    logHdd(LOG_ERROR, "sendToMounter: sendto failed - errno: %d", errno);
    } else {
        logHdd(LOG_DEBUG, "sendToMounter: sent to mounter: %s", jsonString.c_str());
    }
...
void Utils::createFloppyTestImage(void)
{
    ...
    if(!f) {
        logHdd(LOG_ERROR, "Failed to create floppy test image!");
