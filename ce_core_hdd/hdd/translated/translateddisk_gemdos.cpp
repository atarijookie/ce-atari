...
void TranslatedDisk::onDsetdrv(uint8_t *cmd)
{
    ...
    if(newDrive > 15) {                             // drive number out of range? not handled
        logHdd(LOG_ERROR, "TranslatedDisk::onDsetdrv -- new drive index: %d (letter %c) -> out of range, FAIL", newDrive, newDrive + 'A');
    ...
    if(newDrive < 2) {                              // floppy drive selected? store current drive, but don't handle
        ...
        logHdd(LOG_DEBUG, "TranslatedDisk::onDsetdrv -- new drive index: %d (letter %c) -> it's a floppy, not handled, current drive changed", newDrive, currentDriveLetter);
    ...
    if(conf[newDrive].enabled) {                    // if that drive is enabled in cosmosEx
        ...
        logHdd(LOG_DEBUG, "TranslatedDisk::onDsetdrv -- new drive index: %d (letter %c) -> is enabled, current drive: %s", newDrive, currentDriveLetter, (changed ? "really changed" : "was already it, not changed"));

        dataTrans->setStatus(E_OK);                 // return OK
        return;
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onDsetdrv -- new drive index: %d (letter %c) -> not handled", newDrive, newDrive + 'A');
...
void TranslatedDisk::onDgetdrv(uint8_t *cmd)
{
    ...
    if(conf[currentDriveIndex].enabled) {           // if we got this drive, return the current drive
        logHdd(LOG_DEBUG, "TranslatedDisk::onDgetdrv -- current drive is enabled");

        dataTrans->setStatus(currentDriveIndex);
        return;
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onDgetdrv -- current drive not enabled, not handled");
...
void TranslatedDisk::onDsetpath(uint8_t *cmd)
{
    ...
    if(!conf[currentDriveIndex].enabled) {
        logHdd(LOG_DEBUG, "TranslatedDisk::onDsetpath - current Drive Index - %d, not enabled, so not handled", currentDriveIndex);
        dataTrans->setStatus(E_NOTHANDLED);         // if we don't have this, not handled
        return;
    }
    ...
    if(!res) {                                      // failed to get data? internal error!
        logHdd(LOG_ERROR, "TranslatedDisk::onDsetpath - failed to receive data...");
    ...
    if(!res) {                                      // the path doesn't bellong to us?
        logHdd(LOG_ERROR, "TranslatedDisk::onDsetpath - newAtariPath: %s, createFullAtariPath failed!", newAtariPath.c_str());
    ...
    if(waitingForMount) {                           // if the path will be available in a while, but we're waiting for mount to finish now
        logHdd(LOG_DEBUG, "TranslatedDisk::onDsetpath -- waiting for mount, call this function again to succeed later.");
    ...
    if(!hostPathExists(hostPath, true)) {           // path doesn't exists?
        logHdd(LOG_ERROR, "TranslatedDisk::onDsetpath - newAtariPath: %s, hostPathExist failed for %s", newAtariPath.c_str(), hostPath.c_str());

        dataTrans->setStatus(EPTHNF);               // path not found
        return;
    }

    //logHdd(LOG_DEBUG, "TranslatedDisk::onDsetpath - newAtariPath: %s -> hostPath: %s", newAtariPath.c_str(), hostPath.c_str());
    ...
    if(currentDriveIndex != atariDriveIndex) {      // if we need to change the drive too
        ...
        logHdd(LOG_DEBUG, "TranslatedDisk::onDsetpath - current drive changed to %c", currentDriveLetter);
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onDsetpath - newAtariPath: %s, host path: %s, fullAtariPath: %s - success", newAtariPath.c_str(), hostPath.c_str(), fullAtariPath.c_str());
...
void TranslatedDisk::onDgetpath(uint8_t *cmd)
{
    ...
    if(!conf[whichDrive].enabled) {
        logHdd(LOG_DEBUG, "TranslatedDisk::onDgetpath - which drive: %d - not enabled!", whichDrive);
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onDgetpath - which drive: %d, atari path: %s", whichDrive, aPath.c_str());
...
void TranslatedDisk::onFsfirst(uint8_t *cmd)
{
    ...
    if(!res) {                                      // the path doesn't bellong to us?
        logHdd(LOG_ERROR, "TranslatedDisk::onFsfirst - atari search string: %s -- failed to create host path", atariSearchString.c_str());
    ...
    if(waitingForMount) {                           // if the path will be available in a while, but we're waiting for mount to finish now
        logHdd(LOG_DEBUG, "TranslatedDisk::onFsfirst -- waiting for mount, call this function again to succeed later.");

        dataTrans->setStatus(E_WAITING_FOR_MOUNT);
        return;
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onFsfirst - atari search string: %s, host search string: %s", atariSearchString.c_str(), hostSearchString.c_str());
    ...
    if(!res) {
        logHdd(LOG_ERROR, "TranslatedDisk::onFsfirst - host search string: %s -- failed to build gemdos find storage data", hostSearchString.c_str());
    ...
    if (tempFindStorage.count == 0) {
        logHdd(LOG_DEBUG, "TranslatedDisk::onFsfirst - host search string: %s 0 entries found", hostSearchString.c_str());

        dataTrans->setStatus(EFILNF);                               // file not found
        return;
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onFsfirst - host search string: %s -- found %d dir entries", hostSearchString.c_str(), tempFindStorage.count);
    ...
    if(index != -1) {                                                   // got the DTA, reuse it
        logHdd(LOG_DEBUG, "TranslatedDisk::onFsfirst - DTA %08x is in findStorage, reusing...", dta);
    } else {
        logHdd(LOG_DEBUG, "TranslatedDisk::onFsfirst - DTA %08x not in findStorage, will try to find empty findStorage slot", dta);

        index = getEmptyFindStorageIndex();

        if(index == -1) {
            logHdd(LOG_ERROR, "TranslatedDisk::onFsfirst - failed to find empty storage slot!");
            dataTrans->setStatus(EFILNF);                               // file not found
            return;
        }
    }
...
void TranslatedDisk::onFsnext(uint8_t *cmd)
{
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onFsnext -- DTA: %08x, dirIndex: %d", dta, dirIndex);

    int index = getFindStorageIndexByDta(dta);                  // now see if we have findStorage for this DTA
    if(index == -1) {                                           // not found?
        logHdd(LOG_DEBUG, "TranslatedDisk::onFsnext(%08x) - the findBuffer for this DTA not found!", dta);
    ...
    if(dtaRemaining == 0) {                                     // nothing more to transfer?
        logHdd(LOG_DEBUG, "TranslatedDisk::onFsnext(%08x) - no more DTA remaining", dta);
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onFsnext(%08x) - sending %d DTAs to ST", dta, dtaToSend);
...
void TranslatedDisk::onFsnext_last(uint8_t *cmd)                   // after last Fsnext() call this to release the findStorage
{
    ...
    int index = getFindStorageIndexByDta(dta);                  // now see if we have findStorage for this DTA
    if(index == -1) {                                           // not found?
        logHdd(LOG_DEBUG, "TranslatedDisk::onFsnext_last(%08x) - the findBuffer for this DTA not found!", dta);
...
void TranslatedDisk::onDfree(uint8_t *cmd)
{
    ...
    if(res == 0) {
        ...
        logHdd(LOG_DEBUG, "TranslatedDisk::onDfree - free: %lu, total: %lu, spc: %d", clustersFree, clustersTotal, (int) sectorsPerCluster);
    } else {
        logHdd(LOG_ERROR, "TranslatedDisk::onDfree - statvfs FAIL");
    }
...
void TranslatedDisk::onDcreate(uint8_t *cmd)
{
    ...
    if(!res) {                                      // the path doesn't bellong to us?
        logHdd(LOG_ERROR, "TranslatedDisk::onDcreate - newAtariPath: %s -- createFullAtariPath failed", newAtariPath.c_str());
    ...
    if(waitingForMount) {                           // if the path will be available in a while, but we're waiting for mount to finish now
        logHdd(LOG_DEBUG, "TranslatedDisk::onDcreate -- waiting for mount, call this function again to succeed later.");
    ...
    if(status == 0) {                               // directory created?
        logHdd(LOG_DEBUG, "TranslatedDisk::onDcreate - newAtariPath: %s -> hostPath: %s -- dir created", newAtariPath.c_str(), hostPath.c_str());
    ...
    if(status == EEXIST || status == EACCES) {      // path already exists or other access problem?
        logHdd(LOG_ERROR, "TranslatedDisk::onDcreate - newAtariPath: %s -> hostPath: %s -- failed to create dir", newAtariPath.c_str(), hostPath.c_str());
...
void TranslatedDisk::onDdelete(uint8_t *cmd)
{
    ...
    if(!res) {                                      // the path doesn't bellong to us?
        logHdd(LOG_ERROR, "TranslatedDisk::onDdelete - newAtariPath: %s -- createFullAtariPath failed, the path doesn't bellong to us", newAtariPath.c_str());
    ...
    if(waitingForMount) {                           // if the path will be available in a while, but we're waiting for mount to finish now
        logHdd(LOG_DEBUG, "TranslatedDisk::onDdelete -- waiting for mount, call this function again to succeed later.");
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onDdelete - deleting directory hostPath: %s, result is %d", hostPath.c_str(), ires);
...
void TranslatedDisk::onFrename(uint8_t *cmd)
{
    ...
    if(wfm1) {                                                          // if the path will be available in a while, but we're waiting for mount to finish now
        logHdd(LOG_DEBUG, "TranslatedDisk::onFrename -- waiting for mount, call this function again to succeed later.");
    ...
    if(!res || !res2) {                                             // the path doesn't bellong to us?
        logHdd(LOG_ERROR, "TranslatedDisk::onFrename - failed to createFullAtariPath for %s or %s", oldAtariName.c_str(), newAtariName.c_str());

        dataTrans->setStatus(E_NOTHANDLED);                         // if we don't have this, not handled
        return;
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onFrename - rename %s to %s", oldHostName.c_str(), newHostName.c_str());
...
void TranslatedDisk::onFdelete(uint8_t *cmd)
{
    ...
    if(!res) {                                      // the path doesn't bellong to us?
        logHdd(LOG_ERROR, "TranslatedDisk::onFdelete - %s - createFullAtariPath failed", newAtariPath.c_str());
    ...
    if(waitingForMount) {                           // if the path will be available in a while, but we're waiting for mount to finish now
        logHdd(LOG_DEBUG, "TranslatedDisk::onFdelete  -- waiting for mount, call this function again to succeed later.");
    ...
    if(res != 0) {
        logHdd(LOG_ERROR, "TranslatedDisk::onFdelete() -- stat(%s) failed", hostPath.c_str());
        dataTrans->setStatus(EINTRN);
        return;
    }
    ...
    if(isDir) {                                                     // if it's a directory, return FILE NOT FOUND
        logHdd(LOG_ERROR, "TranslatedDisk::onFdelete() -- can't use Fdelete() to delete a directory!");
        dataTrans->setStatus(EFILNF);
        return;
    }
    ...
    if(res == 0) {                                  // file deleted?
        logHdd(LOG_DEBUG, "TranslatedDisk::onFdelete - %s - deleted, success", hostPath.c_str());

        dataTrans->setStatus(E_OK);
        return;
    }

    logHdd(LOG_ERROR, "TranslatedDisk::onFdelete - %s - unlink failed", hostPath.c_str());
...
void TranslatedDisk::onFattrib(uint8_t *cmd)
{
    ...
    if(waitingForMount) {                           // if the path will be available in a while, but we're waiting for mount to finish now
        logHdd(LOG_DEBUG, "TranslatedDisk::onFattrib -- waiting for mount, call this function again to succeed later.");
    ...
    if(res != 0) {
        logHdd(LOG_ERROR, "TranslatedDisk::onFattrib() -- stat(%s) failed", hostName.c_str());
        dataTrans->setStatus(EINTRN);
        return;
    }
    ...
    if(setNotInquire) {     // SET attribs? - SET FAT Attributes!
        if(hostIsFAT) {
            ...
            if(fd < 0) {
                logHdd(LOG_ERROR, "TranslatedDisk::onFattrib() -- setting attributes open(%s) failed", hostName.c_str());
                dataTrans->setStatus(EACCDN);
                return;
            }
            ...
            if(ioctl(fd, FAT_IOCTL_SET_ATTRIBUTES, &dosattrs) < 0) {
                logHdd(LOG_ERROR, "TranslatedDisk::onFattrib() -- ioctl(%s, FAT_IOCTL_SET_ATTRIBUTES, %x) failed errno %d", hostName.c_str(), (int)dosattrs, errno);
            } else {
                logHdd(LOG_DEBUG, "TranslatedDisk::onFattrib() -- FAT attributes %x set %s", (int)dosattrs, hostName.c_str());
            }
            close(fd);
        }
    /*
...
void TranslatedDisk::onFcreate(uint8_t *cmd)
{
    ...
    if(!res) {                                                      // the path doesn't bellong to us?
        logHdd(LOG_ERROR, "TranslatedDisk::onFcreate - %s - createFullAtariPath failed", atariName.c_str());
    ...
    if(waitingForMount) {                           // if the path will be available in a while, but we're waiting for mount to finish now
        logHdd(LOG_DEBUG, "TranslatedDisk::onFcreate -- waiting for mount, call this function again to succeed later.");
    ...
    if(index == -1) {                                               // no place for new file? No more handles.
        logHdd(LOG_DEBUG, "TranslatedDisk::onFcreate - %s - no empty slot", atariName.c_str());
    ...
    if(!f) {
        ...
        logHdd(LOG_ERROR, "TranslatedDisk::onFcreate - %s - fopen failed : %s (ERROR %d)", hostName.c_str(), strerror(errno), status);
    ...
    if(fd >= 0) {
        ...
        if(ioctl(fd, FAT_IOCTL_SET_ATTRIBUTES, &dosattrs) < 0) {
            logHdd(LOG_ERROR, "TranslatedDisk::onFcreate -- failed to set (FAT) files attributes on %s", hostName.c_str());
        } else {
            logHdd(LOG_DEBUG, "TranslatedDisk::onFcreate -- attributes %x set on %s", (int)dosattrs, hostName.c_str());
        }
    }
...
*/
    ...
    if(!f) {
        logHdd(LOG_ERROR, "TranslatedDisk::onFcreate - %s - fopen failed for reopening", hostName.c_str());

        dataTrans->setStatus(EACCDN);                               // if failed to create, access error
        return;
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onFcreate - %s - success, index is: %d", hostName.c_str(), index);
...
void TranslatedDisk::onFopen(uint8_t *cmd)
{
    ...
    if(!res) {                                                      // the path doesn't bellong to us?
        logHdd(LOG_ERROR, "TranslatedDisk::onFopen - %s - createFullAtariPath failed", atariName.c_str());
    ...
    if(waitingForMount) {                           // if the path will be available in a while, but we're waiting for mount to finish now
        logHdd(LOG_DEBUG, "TranslatedDisk::onFopen -- waiting for mount, call this function again to succeed later.");
    ...
    if((mode & 0x07) != 0) {      // if it's WRITE mode and read only, quit
        logHdd(LOG_ERROR, "TranslatedDisk::onFopen - %s - fopen mode is write, but file is read only, fail! ", atariName.c_str());
    ...
    if(index == -1) {                                               // no place for new file? No more handles.
        logHdd(LOG_ERROR, "TranslatedDisk::onFopen - %s - no empty slot, failed", atariName.c_str());
    ...
    if(justRead) {                              // if we should just read from the file (not write and thus create if does not exist)
        if(!hostPathExists(hostName, true)) {   // and the file does not exist
            logHdd(LOG_ERROR, "TranslatedDisk::onFopen - %s - fopen mode is just read, but the file does not exist, failed!", hostName.c_str());
    ...
    if(!f) {
        logHdd(LOG_ERROR, "TranslatedDisk::onFopen - %s - fopen failed!", hostName.c_str());

        dataTrans->setStatus(EACCDN);                               // if failed to create, access error
        return;
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onFopen - %s - success, index is %d", hostName.c_str(), index);
...
void TranslatedDisk::onFclose(uint8_t *cmd)
{
    ...
    if(index == -1) {                                               // handle not found? not handled, try somewhere else
        logHdd(LOG_DEBUG, "TranslatedDisk::onFclose - handle %d not found, not handled", handle);
    ...
    if(!res) {                                                      // failed to get data? internal error!
        logHdd(LOG_ERROR, "TranslatedDisk::onFclose - failed to receive data...");
        dataTrans->setStatus(EINTRN);
        return;
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onFclose - closing handle %d (index %d)", handle, index);
...
void TranslatedDisk::onFdatime(uint8_t *cmd)
{
    ...
    if(setNotGet) {                                                 // on SET
        ...
        logHdd(LOG_DEBUG, "TranslatedDisk::onFdatime %hu %hu => %d (%04d-%02d-%02d %02d:%02d:%02d)",
        ...
        if(res != 0) {
            logHdd(LOG_ERROR, "TranslatedDisk::appendFoundToFindStorage -- stat(%s) failed", files[index].hostPath.c_str());
            dataTrans->setStatus(EINTRN);
            return;
        }
...
void TranslatedDisk::onFread(uint8_t *cmd)
{
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onFread - atariHandle: %d, byteCount: %d, seekOffset: %d", (int) atariHandle, (int) byteCount, (int) seekOffset);

    if(index == -1) {                                               // handle not found? not handled, try somewhere else
        logHdd(LOG_DEBUG, "TranslatedDisk::onFread - atari handle %d not found, not handling", atariHandle);
    ...
    if(byteCount > (254 * 512)) {                                   // requesting to transfer more than 254 sectors at once? fail!
        logHdd(LOG_ERROR, "TranslatedDisk::onFread - trying to transfer more than MAX SECTORS in one transfer (byteCount: %d)", byteCount);
    ...
    if(seekOffset != 0) {                                           // if we should seek before read
        ...
        if(res != 0) {                                              // if seek failed
            logHdd(LOG_ERROR, "TranslatedDisk::onFread - fseek %d failed", seekOffset);
    ...
    if(cnt == byteCount) {                                  // if we read data count as requested
        logHdd(LOG_DEBUG, "TranslatedDisk::onFread - all %d bytes transfered", byteCount);

        dataTrans->setStatus(RW_ALL_TRANSFERED);
        return;
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onFread - only %d bytes out of %d bytes transfered", cnt, byteCount);
...
void TranslatedDisk::onFwrite(uint8_t *cmd)
{
    ...
    if(index == -1) {                                               // handle not found? not handled, try somewhere else
        logHdd(LOG_ERROR, "TranslatedDisk::onFwrite - atariHandle %d not found, not handled", atariHandle);
    ...
    if(byteCount > (254 * 512)) {                                   // requesting to transfer more than 254 sectors at once? fail!
        logHdd(LOG_ERROR, "TranslatedDisk::onFwrite - trying to transfer more than MAX SECTORS count of bytes (%d)", byteCount);
    ...
    if(!res) {                                                  // failed to get data? internal error!
        logHdd(LOG_ERROR, "TranslatedDisk::onFwrite - failed to get data from Hans");
    ...
    if(bWritten != byteCount) {                                 // when didn't write all the data
        logHdd(LOG_DEBUG, "TranslatedDisk::onFwrite - didn't write all data - only %d bytes out of %d were written", bWritten, byteCount);

        dataTrans->setStatus(RW_PARTIAL_TRANSFER);
        return;
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onFwrite - all %d bytes were written", bWritten);
...
void TranslatedDisk::onFseek(uint8_t *cmd)
{
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onFseek - atariHandle: %d, offset: %d, seekMode: %d", (int) atariHandle, (int) offset, (int) seekMode);

    if(index == -1) {                                               // handle not found? not handled, try somewhere else
        logHdd(LOG_DEBUG, "TranslatedDisk::onFseek - atariHandle %d not found, not handled", atariHandle);
    ...
    if(iRes != 0) {                         // on ERROR
        logHdd(LOG_ERROR, "TranslatedDisk::onFseek - fseek %d, %d failed", offset, hostSeekMode);
    ...
    if(pos == -1) {                                                 // failed to get position?
        logHdd(LOG_ERROR, "TranslatedDisk::onFseek - ftell failed");
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onFseek - ok, current position is %d, and we got %d bytes to end of file", pos, (int) bytesToEnd);
...
void TranslatedDisk::onMediach(uint8_t *cmd)
{
    ...
    for(int i=2; i<MAX_DRIVES; i++) {       // create media changed bits
        if(conf[i].enabled && conf[i].mediaChanged) {
            ...
            logHdd(LOG_DEBUG, "TranslatedDisk::onMediach() - drive %c: changed", 'A' + i);
            chgCount++;
        }
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::onMediach() - drives changed count: %d", chgCount);
...
void TranslatedDisk::getByteCountToEndOfFile(uint8_t *cmd)
{
    ...
    if(index == -1) {                               // handle not found? not handled, try somewhere else
        logHdd(LOG_DEBUG, "TranslatedDisk::getByteCountToEndOfFile - atari handle %d not found, not handling", atariHandle);
    ...
    //-----------
    // now send it to ST
    logHdd(LOG_DEBUG, "TranslatedDisk::getByteCountToEndOfFile - for atari handle %d - there are %d bytes until the end of file", atariHandle, (int) bytesToEnd);
...
void TranslatedDisk::onTestWrite(uint8_t *cmd)
{
    ...
    if(!res) {
        logHdd(LOG_ERROR, "TranslatedDisk::onTestWrite - failed to receive data...");
        dataTrans->setStatus(EINTRN);
        return;
    }
    ...
    for(i=0; i<byteCount; i += 2) {                     // verify all data words
        ...
        if(valSt != valGen) {                           // data mismatch? fail
            logHdd(LOG_ERROR, "TranslatedDisk::onTestWrite - data check failed on byte %d out of %d, values: %04X != %04X", i, byteCount, valSt, valGen);
            ...
            for(i=start; i<(start + 12); i += 2) {      // put 6 uint16_t values into log to see them
                uint16_t valSt  = Utils::getWord(dataBuffer + i);
                logHdd(LOG_DEBUG, "  [%d] %04X %s", i, valSt, (i == iFail) ? "<--- BAD" : "");
            }
...
void TranslatedDisk::onSetACSIids(uint8_t *cmd)
{
    ...
    if(!res) {                                              // failed to get data? internal error!
        logHdd(LOG_ERROR, "TranslatedDisk::onSetACSIids - failed to receive data...");
        dataTrans->setStatus(EINTRN);
        return;
    }
