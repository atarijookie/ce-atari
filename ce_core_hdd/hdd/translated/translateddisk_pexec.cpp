...
void TranslatedDisk::onPexec_createImage(uint8_t *cmd)
{
    ...
    if(!res) {
        logHdd(LOG_ERROR, "TranslatedDisk::onPexec_createImage() - failed to receive data...");
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onPexec_createImage() - mode: %04x, path: %s", mode, (char *) dataBuffer + 2);
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onPexec_createImage() - will fake raw drive %c:", 'A' + atariDriveIndex);
    ...
    if(pexecDriveIndex >= 0 && pexecDriveIndex < MAX_DRIVES) {      // if index valid, drive changed
        ...
        logHdd(LOG_DEBUG, "TranslatedDisk::onPexec_createImage() - drive %c: seems to be invalid!", 'A' + pexecDriveIndex);
    }

    if(!res) {                                                      // the path doesn't bellong to us?
        logHdd(LOG_ERROR, "TranslatedDisk::onPexec_createImage() - %s - createFullAtariPath failed", atariName.c_str());
    ...
    if(waitingForMount) {                                           // if the path will be available in a while, but we're waiting for mount to finish now
        logHdd(LOG_DEBUG, "TranslatedDisk::onPexec_createImage() -- waiting for mount, call this function again to succeed later.");
    ...
    if(!hostPathExists(hostName, true)) {                           // and the file does not exist, quit with FILE NOT FOUND
        logHdd(LOG_ERROR, "TranslatedDisk::onPexec_createImage() - %s - fopen mode is just read, but the file does not exist, failed!", hostName.c_str());
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onPexec_createImage() - pexecPrgPath: %s, pexecFakeRootPath: %s", pexecPrgPath.c_str(), pexecFakeRootPath.c_str());
    ...
    if(!f) {
        logHdd(LOG_ERROR, "TranslatedDisk::onPexec_createImage() - %s - fopen failed!", hostName.c_str());
	...
	if(ires != 0) {
		logHdd(LOG_ERROR, "TranslatedDisk::onPexec_createImage -- stat() failed, errno %d", errno);
	}
    ...
    if(fileSizeBytes > PEXEC_DRIVE_USABLE_SIZE_BYTES) {
        fclose(f);
        logHdd(LOG_DEBUG, "TranslatedDisk::onPexec_createImage() - the file is too big to fit in our image!");
...
void TranslatedDisk::createImage(std::string &fullAtariPath, FILE *f, int fileSizeBytes, uint16_t atariTime, uint16_t atariDate)
{
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::createImage() - fullAtariPath: %s, fileSizeBytes: %d, fileSizeSectors: %d", fullAtariPath.c_str(), fileSizeBytes, fileSizeSectors);
    ...
    while(1) {
        ...
        if(strings[found].length() > 0) {           // valid string?
            logHdd(LOG_DEBUG, "TranslatedDisk::createImage() - valid string: %s", strings[found].c_str());
            found++;
        } else {
            logHdd(LOG_DEBUG, "TranslatedDisk::createImage() - invalid string skipped");
        }
    ...
    if(found < 1) {                                 // couldn't break it to string? fail
        logHdd(LOG_DEBUG, "TranslatedDisk::createImage() - could not explode fullAtariPath: %s", fullAtariPath.c_str());
        dataTrans->setStatus(EINTRN);
        return;
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::createImage() - fullAtariPath: %s exploded into %d pieces", fullAtariPath.c_str(), found);

    //--------------
    // first add dirs to image as dir entries
    for(i=0; i<(found-1); i++) {
        logHdd(LOG_DEBUG, "TranslatedDisk::createImage() - LOOP storing DIR ENTRY: %s on curSectorAbs: %d, curSectorRel: %d", strings[i].c_str(), curSectorAbs, curSectorRel);

        createDirEntry(isRootDir, true, atariDate, atariTime, 0, strings[i].c_str(), curSectorAbs, curSectorRel);   // store DIR entry
...
#ifdef PEXEC_FULL_PATH
    logHdd(LOG_DEBUG, "TranslatedDisk::createImage() - LAST storing DIR ENTRY: %s on curSectorAbs: %d, curSectorRel: %d", strings[found - 1].c_str(), curSectorAbs, curSectorRel);
#else
    found = 1;
    strings[0] = pexecPrgFilename;

    logHdd(LOG_DEBUG, "TranslatedDisk::createImage() - storing PRG as DIR ENTRY to root: %s on curSectorAbs: %d, curSectorRel: %d", strings[0].c_str(), curSectorAbs, curSectorRel);
#endif

    createDirEntry(isRootDir, false, atariDate, atariTime, fileSizeBytes, strings[found - 1].c_str(), curSectorAbs, curSectorRel);
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::createImage() - storing PRG on relative sectors %d - %d (absolute starting sector: %d)", prgSectorStart, prgSectorEnd, curSectorAbs);
...
void TranslatedDisk::storeFatChain(uint8_t *pbFat, uint16_t sectorStart, uint16_t sectorEnd)
{
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::storeFatChain() - storing chain %d .. %d", sectorStart, sectorEnd);
...
{
    logHdd(LOG_DEBUG, "TranslatedDisk::onPexec_getBpb() - PEXEC_FAT_SECTORS_NEEDED: %d, PEXEC_DRIVE_SIZE_SECTORS: %d", PEXEC_FAT_SECTORS_NEEDED, PEXEC_DRIVE_SIZE_SECTORS);
...
void TranslatedDisk::onPexec_readSector(uint8_t *cmd)
{
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onPexec_readSector() - startingSector: %d, sectorCount: %d", startingSector, sectorCount);

    if(startingSector + sectorCount > PEXEC_DRIVE_SIZE_SECTORS) {       // would be out of boundary? fail
        logHdd(LOG_ERROR, "TranslatedDisk::onPexec_readSector() - out of range!");
...
void TranslatedDisk::onPexec_writeSector(uint8_t *cmd)
{
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::onPexec_writeSector() - startingSector: %d, sectorCount: %d", startingSector, sectorCount);

    if(startingSector + sectorCount > PEXEC_DRIVE_SIZE_SECTORS) {       // would be out of boundary? fail
        logHdd(LOG_ERROR, "TranslatedDisk::onPexec_writeSector() - out of range!");
    ...
    if(!res) {
        logHdd(LOG_ERROR, "TranslatedDisk::onPexec_writeSector() - failed to receive data...");
        dataTrans->setStatus(EINTRN);
        return;
    }
