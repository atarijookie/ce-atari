...
void Scsi::findAttachedDisks(void)
{
    ...
    for(int i=0; i<8; i++) {                        // go through all the possible drives
        ...
        logHdd(LOG_DEBUG, "Scsi::findAttachedDisks() - ID %d - isBoot: %d, isFile: %d, isDev: %d", i, isBoot, isFile, isDev);
        ...
        if(isFile) {        // file means image
            ...
            logHdd(LOG_DEBUG, "Scsi::findAttachedDisks() - ID %d -> IMAGE", i);
            ...
            logHdd(LOG_DEBUG, "Scsi::findAttachedDisks() - ID %d -> DEVICE", i);
            ...
            logHdd(LOG_DEBUG, "Scsi::findAttachedDisks() - ID %d -> TranslatedBoot", i);
            ...
            logHdd(LOG_WARNING, "Scsi::findAttachedDisks() -- unknown source type!");
        }
    }
}
...
void Scsi::processCommand(uint8_t *command)
{
    ...
    if(dataTrans == 0) {
        logHdd(LOG_ERROR, "Scsi::processCommand was called without valid dataTrans, can't tell ST that his went wrong...");
        return;
    }
    ...
    if(dataMedia == 0) {
        logHdd(LOG_ERROR, "Scsi::processCommand was called without valid dataMedia, will return error CHECK CONDITION acsiId=%d", acsiId);
    ...
    logHdd(LOG_DEBUG, "Scsi::processCommand -- isIcd: %d, ACSI ID: %d, LUN: %d, CMD: 0x%02x - %s", isIcd, acsiId, lun, justCmd, getCommandName(justCmd));
    ...
    if(lun != 0) {      // if LUN is not zero, the command is invalid
        if(justCmd == SCSI_C_REQUEST_SENSE) {                   // special handling in REQUEST SENSE
            ...
            logHdd(LOG_ERROR, "Scsi::processCommand() - non-zero LUN, failing");
        }
    ...
    if(sendDataAndStatus_notJustStatus) {                           // if this is set, let acsiDataTrans send data and status (like in most of the code)
        logHdd(LOG_DEBUG, "Scsi::processCommand -- now the acsiDataTrans will handle transfer to Hans");
        dataTrans->sendDataAndStatus();                             // send all the stuff after handling, if we got any
    } else {                                                        // if data was already sent (large blocks in readSectors() or writeSectors()), we just need to send the status
        logHdd(LOG_DEBUG, "Scsi::processCommand -- data was already sent to Hans, just send status (0x%02x) to Hans", devInfo[acsiId].LastStatus);
...
void Scsi::ProcICD(uint8_t lun, uint8_t justCmd)
{
    ...
    if(!dataMedia->isInit())
    {
        ...
        if((justCmd == SCSI_C_READ10) || (justCmd == SCSI_C_WRITE10) || (justCmd == SCSI_C_READ_CAPACITY))
        {
            ...
            logHdd(LOG_DEBUG, "Scsi::ProcICD - dataMedia is not init, returning status according to init");
            return;
        }
    }
    //----------------
    ...
    if(dataMedia->mediaChanged())
    {
        if(justCmd != SCSI_C_INQUIRY)
        {
            ...
            logHdd(LOG_DEBUG, "Scsi::ProcICD - media changed, and this isn't INQUIRY command, returning UNIT ATTENTION");
            return;
        }
    }
    ...
    if(devInfo[acsiId].accessType == SCSI_ACCESSTYPE_READ_ONLY) {
        if(justCmd == SCSI_C_WRITE10) {
            returnInvalidCommand();
            logHdd(LOG_ERROR, "Scsi::ProcICD - tried to WRITE on READ ONLY media, fail");
            return;
        }
    }
    ...
    if(devInfo[acsiId].accessType == SCSI_ACCESSTYPE_NO_DATA) {
        if(justCmd == SCSI_C_WRITE10 || justCmd == SCSI_C_READ10 || justCmd == SCSI_C_VERIFY) {
            returnInvalidCommand();
            logHdd(LOG_ERROR, "Scsi::ProcICD - READ / WRITE / VERIFY on empty media, fail");
            return;
        }
    }
    //----------------
...
void Scsi::updateTranslatedBootMedia(void)
{
    ...
    if(idOnBus == -1) {         // don't have this media?
        logHdd(LOG_ERROR, "Scsi::updateTranslatedBootMedia() - could not find CE_DD bus ID, fail");
        return;
    } else {
        logHdd(LOG_DEBUG, "Scsi::updateTranslatedBootMedia() - the bus ID of CE_DD is: %d", idOnBus);
    }
