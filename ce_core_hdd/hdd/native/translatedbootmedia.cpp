...
bool TranslatedBootMedia::loadDataIntoBuffer(void)
{
    ...
    if(!f) {
        logHdd(LOG_ERROR, "TranslatedBootMedia - failed to open Level 1 bootsector file: %s", PATH_CE_DD_BS_L1.c_str());
        return false;
    }
    logHdd(LOG_DEBUG, "TranslatedBootMedia - loaded Level 1 bootsector file: %s", PATH_CE_DD_BS_L1.c_str());

    bytesRead = fread(&imageBuffer[0], 1, 512, f);

    if(bytesRead != 512) {
        logHdd(LOG_ERROR, "TranslatedBootMedia - didn't read 512 bytes from Level 1 bootsector file: %s", PATH_CE_DD_BS_L1.c_str());
        return false;
    }
    ...
    if(!f) {
        logHdd(LOG_ERROR, "TranslatedBootMedia - failed to open Level 2 bootsector file: %s", PATH_CE_DD_BS_L2.c_str());
        return false;
    }
    ...
    if(bytesRead != 512) {
        logHdd(LOG_ERROR, "TranslatedBootMedia - didn't read 512 bytes from Level 2 bootsector file: %s", PATH_CE_DD_BS_L2.c_str());
        return false;
    }
    ...
    if(!f) {
        logHdd(LOG_ERROR, "TranslatedBootMedia - failed to open %s", PATH_CE_DD_PRG_PATH_AND_FILENAME.c_str());
        return false;
    }
...
void TranslatedBootMedia::updateBootsectorConfig(void)
{
    ...
    if(pos == -1) {
        logHdd(LOG_ERROR, "TranslatedBootMedia - didn't find the config position in bootsector.");
        return;
    }
    ...
    logHdd(LOG_DEBUG, "TranslatedBootMedia - bootsector will read %d sectors, the driver will take %d kB of RAM.", (int) imageBuffer[pos + 7], (int) (totalSize / 1024));
...
void TranslatedBootMedia::updateBootsectorConfigWithACSIid(uint8_t acsiId)
{
    ...
    if(hwConfig.hddIface == HDD_IF_ACSI) {     // for ACSI - it's the ID (0 .. 7)
        ...
        logHdd(LOG_DEBUG, "TranslatedBootMedia::updateBootsectorConfigWithACSIid() - hddIface is SCSI, bootsector ID set to: %d", (int) id);
    }
