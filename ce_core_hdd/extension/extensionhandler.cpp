...
void ExtensionHandler::waitForSignal(uint32_t ms)
{
    ...
    while(1) {
        ...
        if(extSignalReceived) {     // got signal? quit now
            logHdd(LOG_DEBUG, "waitForSignal - signal received after %d ms", diff);
            break;
        }

        if(diff >= ms) {            // timeout? quit
            logHdd(LOG_DEBUG, "waitForSignal timeout after %d ms", diff);
            break;
        }
...
void ExtensionHandler::processCommand(uint8_t *command)
{
    ...
    if(dataTrans == 0) {
        logHdd(LOG_ERROR, "ExtensionHandler::processCommand was called without valid dataTrans, can't tell ST that his went wrong...");
        return;
    }

    dataTrans->clear();         // clean data transporter before handling

    logHdd(LOG_DEBUG, "ExtensionHandler::processCommand -- CMD: %02x %02x %02x %02x %02x %02x - %s",
    ...
    if(justCmd == CMD_CALL_RAW_WRITE || justCmd == CMD_CALL_LONG_WRITE_ARGS) {
        ...
        if(byteCountInDataBuffer > 0) {
            logHdd(LOG_DEBUG, "justCmd: %02x, will receive %d bytes first", justCmd, byteCountInDataBuffer);
        ...
        logHdd(LOG_DEBUG, "justCmd: %02x, the data read phase will expect %d bytes", justCmd, expectedBytesOnRead);
    }
    ...
    logHdd(LOG_DEBUG, "ExtensionHandler::processCommand -- send data and status after handling");
...
void ExtensionHandler::cexOpen(void)
{
    ...
    for(int i=0; i<MAX_EXTENSIONS_OPEN; i++) {
        if(extensions[i].nameMatching(extName)) {   // supplied name matches one of our open extensions?
            extensions[i].touch();
            logHdd(LOG_DEBUG, "ExtensionHandler::cexOpen - extension %s found at index %d", extName, i);
    ...
    logHdd(LOG_DEBUG, "ExtensionHandler::cexOpen - will use index %d for extension %s", extensionId, extName);
    ...
    if(ext->state != EXT_STATE_NOT_RUNNING) {               // if the extension is running or starting, stop it now
        logHdd(LOG_DEBUG, "ExtensionHandler::cexOpen - previous extension at index %d starting / running, so stoping it now", extensionId);
    ...
    logHdd(LOG_DEBUG, "ExtensionHandler::cexOpen - startScriptPath: '%s', startScriptCommand: '%s'", startScriptPath.c_str(), startScriptCommand.c_str());
    ...
    if(Utils::fileExists(startScriptPath)) {
        ...
        logHdd(LOG_DEBUG, "ExtensionHandler::cexOpen - now executing '%s'", startScriptCommand.c_str());
    ...
    logHdd(LOG_DEBUG, "ExtensionHandler::cexOpen - start script does not exist, so sending JSON: '%s' to TaskQ - for extension installation", jsonRequest.c_str());
    ...
    if(ok) {    // sending ok?
        logHdd(LOG_DEBUG, "Sending to TaskQ ok, marking extension state as EXT_STATE_INSTALLING");
        ext->state = EXT_STATE_INSTALLING;
        dataTrans->setStatus(extensionId);          // return index of this extension
    } else {    // sending failed?
        logHdd(LOG_WARNING, "Sending to TaskQ failed, extension not started. Returning STATUS_EXT_ERROR.");
        ext->clear();
...
{
    logHdd(LOG_DEBUG, "ExtensionHandler::cexStatusOrResponse - extensionId: %d, functionId: %d, sectorCount: %d", extensionId, functionId, sectorCount);

    if(extensionId >= MAX_EXTENSIONS_OPEN) {        // bad extension id?
        logHdd(LOG_WARNING, "ExtensionHandler::cexStatusOrResponse - extension index %d out of bounds", extensionId);
    ...
    if(functionId == CEX_FUN_OPEN) {            // if trying to get status of this extension - if it's open, then...
        if(ext->state == EXT_STATE_RUNNING) {   // if extesion is running, send function table
            ...
            logHdd(LOG_DEBUG, "ExtensionHandler::cexStatusOrResponse - signatures have %d bytes, will send %d bytes (based on sector count)", dataLen, sendDataSize);
            dataTrans->addDataBfr(dataBuffer, sendDataSize, true);
            dataTrans->setStatus(STATUS_OK);
        } else {                                // extension not running yet
            logHdd(LOG_DEBUG, "ExtensionHandler::cexStatusOrResponse - extension not running, returning STATUS_EXT_NOT_RUNNING");
    ...
    if(functionId <= CEX_FUN_CLOSE) {           // for fixed (base) functions return not/running status
        ...
        logHdd(LOG_DEBUG, "ExtensionHandler::cexStatusOrResponse - functionId: %d - returning status: %02x", functionId, openCloseStatus);
    ...
    //     logHdd(LOG_WARNING, "ExtensionHandler::cexStatusOrResponse - no response stored for functionId %d (either not called yet, or rewritten by next function call)", functionId);
    //     dataTrans->setStatus(STATUS_BAD_ARGUMENT);
    //     EXT_MUTEX_UNLOCK;
    //     return;
    // }
    ...
    if(resp->state == RESP_STATE_NOT_RECEIVED) {    // if response not received yet, wait for it
        ...
        logHdd(LOG_DEBUG, "ExtensionHandler::cexStatusOrResponse - response not received yet, will now wait");
    ...
    if(resp->state == RESP_STATE_RECEIVED) {        // got the response now? return it
        ...
        if(sendResponseSize < resp->dataLen) {      // not sending whole response - response will be truncated?
            logHdd(LOG_WARNING, "ExtensionHandler::cexStatusOrResponse - response size is %d bytes, but will send only %d bytes to ST - response truncated", resp->dataLen, sendResponseSize);
        } else {
            logHdd(LOG_WARNING, "ExtensionHandler::cexStatusOrResponse - will return data response of %d bytes and status %02x", sendResponseSize, resp->statusByte);
        }
        ...
        logHdd(LOG_DEBUG, "ExtensionHandler::cexStatusOrResponse - response not received yet");
    }
...
void ExtensionHandler::cexClose(uint8_t extensionId)
{
    if(extensionId >= MAX_EXTENSIONS_OPEN) {        // index out of range
        logHdd(LOG_WARNING, "ExtensionHandler::cexClose - extension index %d out of bounds", extensionId);
        dataTrans->setStatus(STATUS_BAD_ARGUMENT);
        return;
    }
    ...
    if(ext->state == EXT_STATE_NOT_RUNNING) {    // if it's not running
        logHdd(LOG_DEBUG, "ExtensionHandler::cexClose - extension at index %d not running, NO OP", extensionId);
        ext->clear();
        dataTrans->setStatus(STATUS_OK);
        EXT_MUTEX_UNLOCK;
        return;
    ...
    if(!Utils::fileExists(stopScriptCommand)) {                 // stop script doesn't exist? fail here
        logHdd(LOG_WARNING, "ExtensionHandler::cexClose - extension at index %d - stop script %s does not exist!", extensionId, stopScriptCommand.c_str());
        ext->clear();
    ...
    logHdd(LOG_DEBUG, "ExtensionHandler::cexClose - extension at index %d - now executing %s", extensionId, stopScriptCommand.c_str());
...
void ExtensionHandler::cexExtensionFunction(uint8_t justCmd, uint8_t extensionId, uint8_t functionId, uint32_t sectorCount)
{
    if(functionId <= CEX_FUN_CLOSE) {       // if trying to call this method with one of the base / fixed functions, fail
        logHdd(LOG_WARNING, "ExtensionHandler::cexExtensionFunction - functionId %d too low for cexExtensionFunction", functionId);
        dataTrans->setStatus(STATUS_BAD_ARGUMENT);
        return;
    }
    ...
    if(extensionId >= MAX_EXTENSIONS_OPEN) {        // bad extension id?
        logHdd(LOG_WARNING, "ExtensionHandler::cexExtensionFunction - extension index %d out of bounds", extensionId);
        dataTrans->setStatus(STATUS_BAD_ARGUMENT);
        return;
    }
    ...
    if(ext->state != EXT_STATE_RUNNING) {           // extension not running?
        logHdd(LOG_WARNING, "ExtensionHandler::cexExtensionFunction - extension at index %d NOT RUNNING", extensionId);
        dataTrans->setStatus(STATUS_EXT_NOT_RUNNING);
        EXT_MUTEX_UNLOCK;
        return;
    }
    ...
    if(functionIndex > MAX_EXPORTED_FUNCTIONS || !ext->functionTable.signatures[functionIndex].used) {
        logHdd(LOG_WARNING, "ExtensionHandler::cexExtensionFunction - functionIndex %d is not used (expoted) or functionIndex %d > %d", functionIndex, functionIndex, MAX_EXPORTED_FUNCTIONS);
        dataTrans->setStatus(STATUS_BAD_ARGUMENT);
        EXT_MUTEX_UNLOCK;
        return;
    }
    ...
    if(justCmd != expectedJustCmd) {        // wrong CMD used? fail
        logHdd(LOG_WARNING, "ExtensionHandler::cexExtensionFunction - expected cmd is %02x, but cmd that came is %02x", expectedJustCmd, justCmd);
        dataTrans->setStatus(STATUS_BAD_ARGUMENT);
        EXT_MUTEX_UNLOCK;
        return;
    }
    ...
    logHdd(LOG_DEBUG, "ExtensionHandler::cexExtensionFunction  extensionId: %d, functionId: %d, functionIndex: %d, functionName: '%s'", extensionId, functionId, functionIndex, functionName);

    if(justCmd == CMD_CALL_RAW_WRITE || justCmd == CMD_CALL_RAW_READ) {     // raw WRITE and raw READ
        logHdd(LOG_DEBUG, "ExtensionHandler::cexExtensionFunction - justCmd: %d, extensionId: %d, will do raw call of function '%s'", justCmd, extensionId, functionName);
        ...
        logHdd(LOG_DEBUG, "ExtensionHandler::cexExtensionFunction - justCmd: %d, extensionId: %d, will do LONG WRITE ARGS call of function '%s'", justCmd, extensionId, functionName);
        ...
        logHdd(LOG_WARNING, "ExtensionHandler::cexExtensionFunction - bad justCmd %d passed to this method", justCmd);
        dataTrans->setStatus(STATUS_BAD_ARGUMENT);
        EXT_MUTEX_UNLOCK;
    }
}
...
uint8_t ExtensionHandler::sendCallRawToExtension(uint8_t extensionId, char* functionName)
{
    ...
    logHdd(LOG_DEBUG, "ExtensionHandler::sendCallRawToExtension - funcCallJson: '%s'", funcCallJson.c_str());
...
{
    logHdd(LOG_DEBUG, "ExtensionHandler::sendCallLongToExtension - extensionId: %d, functionIndex: %d, functionName: '%s'", extensionId, functionIndex, functionName);
        ...
        switch(fs->argumentTypes[i]) {
            ...
            case TYPE_PATH:
            {
                ...
                if(param[1] != ':' || param[2] != '\\') {   // path doesn't start with 'X:\'? not a full path!
                    logHdd(LOG_WARNING, "ExtensionHandler::sendCallLongToExtension - supplied PATH argument is not full path, ldp_shortToLongPath() will return wrong value");
                    ...
                    if(driveNo >= 'A' && driveNo <= 'P') {          // upper case drive letter, valid range - letter to drive number
                        ...
                        logHdd(LOG_WARNING, "ExtensionHandler::sendCallLongToExtension - drive letter '%c' seems wrong!", driveNo);
            ...
            case TYPE_BIN_DATA:
            {
                ...
                logHdd(LOG_DEBUG, "ExtensionHandler::sendCallLongToExtension - binDataLen: %d, EXT_BUFFER_SIZE: %d, using smaller: %d", binDataLen, EXT_BUFFER_SIZE-6, binDataLenVerified);
    ...
    logHdd(LOG_DEBUG, "ExtensionHandler::sendCallLongToExtension - funcCallJson: '%s'", funcCallJson.c_str());
...
bool ExtensionHandler::sendDataToSocket(const char* socketPath, const uint8_t* data, uint32_t dataLen)
{
	...
	if (sockFd < 0) {   // if failed to create socket
	    logHdd(LOG_ERROR, "ExtensionHandler::sendDataToSocket: failed to create socket - errno: %d", errno);
	    return false;
	}

    logHdd(LOG_DEBUG, "ExtensionHandler::sendDataToSocket: opened socket %s", socketPath);
    ...
    if(res < 0) {       // if failed to send
	    logHdd(LOG_ERROR, "ExtensionHandler::sendDataToSocket: sendto failed - errno: %d", errno);
    } else {
        logHdd(LOG_DEBUG, "ExtensionHandler::sendDataToSocket: %d bytes of data sent", dataLen);
    }
