...
void ChipInterfaceNetwork::createListeningSocket(void)
{
    ...
    if ((fdListen = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
        logHdd(LOG_ERROR, "netServer - failed to open socket");
        return;
    }
    ...
    if (setsockopt(fdListen, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt, sizeof(opt))) {
        logHdd(LOG_ERROR, "netServer - setsockopt() failed");
        return;
    }
    ...
    if (bind(fdListen, (struct sockaddr *) &addressListen, sizeof(addressListen)) < 0) {
        logHdd(LOG_ERROR, "netServer - bind() failed");
        return;
    }

    // mark the socket as a passive socket
    if (listen(fdListen, 1) < 0) {
        logHdd(LOG_ERROR, "netServer - listen() failed");
        return;
    }

    logHdd(LOG_INFO, "netServer - listening on tcp port: %d", flags.portClient);
}
...
void ChipInterfaceNetwork::acceptSocketIfNeededAndPossible(void)
{
    ...
    logHdd(LOG_DEBUG, "acceptSocketIfNeededAndPossible() - client connected");
...
void ChipInterfaceNetwork::closeClientSocket(void)
{
    ...
    logHdd(LOG_DEBUG, "closeClientSocket() - client disconnected");
}
...
void ChipInterfaceNetwork::createServerReportSocket(void)
{
    ...
    if(fdReport < 0) {        // on error
        logHdd(LOG_ERROR, "failed to open socket for main server reporting");
        return;
    }
...
bool ChipInterfaceNetwork::actionNeeded(uint8_t *inBuf)
{
    ...
    if(fdClient <= 0) {                 // (still) no client connected? no action needed
        //logHdd(LOG_DEBUG, "actionNeeded() - client not connected yet");
        return false;
    }
    ...
    if(rv < 0 || bytesAvailable <= 0) {                     // ioctl fail or nothing to read? no action needed
        if((now - lastTimeRecv) > 5000) {
            Utils::closeFdIfOpen(fdClient);
            logHdd(LOG_INFO, "actionNeeded() - no client data for some time, disconnecting");
        }

        return false;
    }
    logHdd(LOG_DEBUG, "actionNeeded() - bytesAvailable: %d", bytesAvailable);
    ...
    while(sigintReceived == 0) {
        ...
        //logHdd(LOG_DEBUG, "actionNeeded() - gotAtnId=%d, gotAtnCode=%d", gotAtnId, gotAtnCode);
        ...
        logHdd(LOG_DEBUG, "actionNeeded() - weird situation?");
        break;
    }
...
void ChipInterfaceNetwork::getFWversion(void)
{
    ...
    logHdd(LOG_DEBUG, "FW: %d-%02d-%02d", v.getYear(), v.getMonth(), v.getDay());
}
...
bool ChipInterfaceNetwork::hdd_sendData_start(uint32_t totalDataCount, uint8_t scsiStatus, bool withStatus)
{
    if(totalDataCount > 0xffffff) {
        logHdd(LOG_ERROR, "ChipInterfaceNetwork::hdd_sendData_start -- trying to send more than 16 MB, fail");
        return false;
    }
...
{
    if(totalDataCount > 0xffffff) {
        logHdd(LOG_ERROR, "ChipInterfaceNetwork::hdd_recvData_start() -- trying to send more than 16 MB, fail");
        return false;
    }
...
{
    logHdd(LOG_DEBUG, "hdd_sendStatusToHans - statusByte: %02x", statusByte);
...
bool ChipInterfaceNetwork::waitForAtn(int atnIdWant, uint8_t atnCode, uint32_t timeoutMs, uint8_t *inBuf)
{
    ...
    while(sigintReceived == 0) {
        ...
        if(atnIdGot == NET_ATN_DISCONNECTED) {         // if buffered reader detected client disconnect, close it and quit
            logHdd(LOG_DEBUG, "waitForAtn() - DISCONNECTED!");
        ...
        //logHdd(LOG_DEBUG, "waitForAtn() - atnIdGot=%d, atnCode=%d", atnIdGot, atnCode);
        ...
        //logHdd(LOG_DEBUG, "waitForAtn() - %02X %02X %02X %02X %02X %02X %02X %02X", inBuf[0], inBuf[1], inBuf[2], inBuf[3], inBuf[4], inBuf[5], inBuf[6], inBuf[7]);
...
bool ChipInterfaceNetwork::sendHeaderAndDataToChip(uint16_t cmdCode, uint8_t* data, uint32_t len)        // send header and data to chip
{
    ...
    if(len > 512)       // for larger data send using separate write() commands
    {
        ...
        {
            logHdd(LOG_DEBUG, "sendHeaderAndDataToChip failed!");
            return false;
        }

        good = sendDataToChip(data, len);
        logHdd(LOG_DEBUG, "sendHeaderAndDataToChip - good: %d", good);
    }
...
uint32_t ChipInterfaceNetwork::recvFromClient(uint8_t* buf, int maxLen)
{
    ...
    // if(received > 0) {
    //     logHdd(LOG_DEBUG, "recvFromClient(): %d bytes", received);
...
void ChipInterfaceNetwork::dropRestOfData(void)
{
    while(true)
    {
        ...
        logHdd(LOG_WARNING, "dropping %d unused bytes", dropSize);
    }
}
