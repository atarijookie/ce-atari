...
BYTE dmaDataTx_do_Falcon(BYTE readNotWrite, BYTE *buffer, DWORD dataByteCount)
{
    ...
    *WDC = (dataByteCount >> 9);                    // write sector count (not byte count)
    ...
    DWORD sectorCount   = (dataByteCount    >>  9);     // bytes to sectors
    DWORD mbCount       = (sectorCount      >> 11) + 1; // sectors to mega bytes
    DWORD timeoutSecs   = mbCount * 3;                  // mega bytes to seconds
    DWORD timeoutTics   = timeoutSecs * 200;            // seconds to tics
    
    BYTE res = wait_dma_cmpl(timeoutTics);              // wait for DMA completetion
    if(res) {                                           // failed?
        stopDmaFalcon();
