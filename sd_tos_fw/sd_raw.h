...
struct TSDinfo
{
    ...
    BYTE product[6];                // A product name
    BYTE revision;                  // The card's revision, coded in packed BCD.
    DWORD serial;                   // A serial number assigned by the manufacturer
    BYTE manufacturing_year;        // The year of manufacturing
    BYTE manufacturing_month;       // The month of manufacturing
    DWORD capacity;                 // The card's total capacity in sectors.
    BYTE flag_copy;                 // Defines wether the card's content is original or copied.
    BYTE flag_write_protect;        // Defines wether the card's content is write-protected.
    BYTE flag_write_protect_temp;   // Defines wether the card's content is temporarily write-protected
    BYTE format;                    // The card's data layout
};

BYTE sd_raw_init(void);
BYTE sd_raw_available(void);
BYTE sd_raw_locked(void);
