...
void Ikbd::processFoundDev(const char *linkName, const char *fullPath)
{
    ...
    if(fd < 0) {
        Debug::out(LOG_ERROR, "Failed to open input device %s (%s) : %s ", fullPath, what, strerror(errno));
        return;
    }
...
void Ikbd::processMouse(input_event *ev)
{
    if(ev->type == EV_KEY) {        // on button press
        ...
        if(mouseAbsBtnAct & MOUSEBTN_REPORT_ACTLIKEKEYS) {      // if should report mouse clicks as keys
            ...
            switch(absButtons) {
                case MOUSEABS_BTN_LEFT_DOWN:    bfr = 0x74; chipInterface->ikbdUartWriteToAll(&bfr, 1); break;
                case MOUSEABS_BTN_LEFT_UP:      bfr = 0xf4; chipInterface->ikbdUartWriteToAll(&bfr, 1); break;
                case MOUSEABS_BTN_RIGHT_DOWN:   bfr = 0x75; chipInterface->ikbdUartWriteToAll(&bfr, 1); break;
                case MOUSEABS_BTN_RIGHT_UP:     bfr = 0xf5; chipInterface->ikbdUartWriteToAll(&bfr, 1); break;
            }
        }
...
//    Debug::out(LOG_DEBUG, "processKeyboard");
    ...
    switch(ev->type) {
    case EV_SYN:
        //Debug::out("Ikbd::processKeyboard() EV_SYN code=%04x\n", ev->code);
        break;
    case EV_KEY:
        //Debug::out("Ikbd::processKeyboard() EV_KEY code=%04x ev->value=%d\n", ev->code, ev->value);
        ...
        chipInterface->ikbdUartWriteToAll(&bfr, 1);
        break;
    case EV_MSC:
        //Debug::out("Ikbd::processKeyboard() EV_MSC code=%d value=0x%08x\n", ev->code, ev->value);
        break;
    default:
        Debug::out(LOG_DEBUG, "Ikbd::processKeyboard() ***UNKNOWN*** ev->type=%d ev->code=0x%04x ev->value=0x%08x\n", ev->type, ev->code, ev->value);
    }
}
...
void Ikbd::toggleKeyboardExclusiveAccess(void)
{
    ...
    Debug::out(LOG_DEBUG, "Ikbd::toggleKeyboardExclusiveAccess() fdKeyb=%d, fdMouse=%d, keyboardExclusiveAccess=%d\n", fdKeyb, fdMouse, keyboardExclusiveAccess);
...
void Ikbd::grabExclusiveAccess(int fd)
{
    ...
    if(ioctl(fd, EVIOCGRAB, (void*)1) < 0) { // grab exclusive
        Debug::out(LOG_ERROR, "Ikbd::grabExclusiveAccess() ioctl failed : %s", strerror(errno));
    }
}
...
void Ikbd::releaseExclusiveAccess(int fd)
{
    ...
    if(ioctl(fd, EVIOCGRAB, (void*)0) < 0) { // release
        Debug::out(LOG_ERROR, "Ikbd::releaseExclusiveAccess() ioctl failed : %s", strerror(errno));
    }
}

bool Ikbd::handleHotkeys(int pcKey, bool pressed, bool skipKeyboardTranslation)
{
    Debug::out(LOG_DEBUG, "Ikbd::handleHotkeys - %d, %d, %d", pcKey, pressed, skipKeyboardTranslation);
    ...
    if ((leftShiftsPressed > 0 || rightShiftsPressed > 0) && ctrlsPressed > 0 && pressed) {
        ...
        if (f11sPressed > 0) {
            ...
            Debug::out(LOG_DEBUG, "Ikbd::handleHotkeys - joy0 <-> joy1 [before: %s, now: %s]",
    ...
    if (waitingForHotkeyRelease) {
        ...
        if (oldWaitingForHotkeyRelease == waitingForHotkeyRelease) {
            Debug::out(LOG_DEBUG, "Ikbd::handleHotkeys - hotkey release pending...");
        } else {
            Debug::out(LOG_DEBUG, "Ikbd::handleHotkeys - hotkey release not pending anymore");
        }
        return true;
    } else {
        return false;
