...
void Ikbd::dumpBuffer(bool fromStNotKeyboard)
{
    ...
    Debug::out(LOG_DEBUG, text.c_str());                      // show the hex dump of data
}
...
void Ikbd::processStCommands(void)
{
    ...
    while(cbStCommands.count > 0) {                             // while there are some data, process
        ...
        Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- got command %02x", cmd);

        if(cmd == STCMD_RESET || (cmd & 0x80) == 0) {           // reset or highest bit is 0? SET command
            len = stCommandLen[cmd];                            // try to get the command length
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- got command %02x, len %d", cmd, len);
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- got command %02x (GET), len %d", cmd, len);
        }

        Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- got command %02x, len: %d, cb contains %d bytes", cmd, len, cbStCommands.count);

        if(len == 0) {                                          // it's not GET command and we don't have this SET command defined?
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- not GET cmd, and we don't know what to do");
        ...
        if(cmd == STCMD_MEMORY_LOAD) {                          // special case: this command has length stored on index[3]
            Debug::out(LOG_DEBUG, "Ikbd::processStCommands -- ST says: STCMD_MEMORY_LOAD");
        ...
        if(len > cbStCommands.count) {                          // if we don't have enough data in the buffer, quit
            //Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- not enough data in buffer, quitting (%d > %d)", len, cbStCommands.count);
            return;
        }
        ...
        switch(bfr[0]) {
            ...
            if (bfr[1] != 0x01) {
                Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_RESET but not followed by 0x01, skipping");
                break;
            }

            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_RESET");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_PAUSE_OUTPUT");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_JOYSTICK_EVENT_REPORTING");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_JOYSTICK_INTERROG_MODE");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_JOYSTICK_MONITORING");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_FIRE_BUTTON_MONITORING");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_JOYSTICK_KEYCODE_MODE");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_JOYSTICK_INTERROGATION");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_DISABLE_JOYSTICKS");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_REL_MOUSE_POS_REPORTING");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_MOUSE_KEYCODE_MODE");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_MOUSE_THRESHOLD");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_MOUSE_SCALE");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_ABS_MOUSE_POS_REPORTING");
            ...
            Debug::out(LOG_DEBUG,  "new mouse max: [%04x, %04x]", absMouse.maxX, absMouse.maxY);
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_INTERROGATE_MOUSE_POS");

			if(mouseMode != MOUSEMODE_ABS) {			// this is only valid in absolute mouse mode
                Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- STCMD_INTERROGATE_MOUSE_POS -- not sending anything because not in ABS mouse mode");
				break;
			}

			if(ceIkbdMode == CE_IKBDMODE_INJECT && !gotUsbMouse()) {	// when working in INJECT mode and don't have mouse, don't answer
                Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- STCMD_INTERROGATE_MOUSE_POS -- not sending anything because we're in INJECT mode and we don't have USB mouse");
				break;
			} else {
                Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- STCMD_INTERROGATE_MOUSE_POS -- will send USB mouse position (either we're in SOLO mode and / or we gotUsbMouse() )");
            }
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_LOAD_MOUSE_POS");

            // read max X and Y position values
            absMouse.x = (((uint16_t) bfr[2]) << 8) | ((uint16_t) bfr[3]);
            absMouse.y = (((uint16_t) bfr[4]) << 8) | ((uint16_t) bfr[5]);

            Debug::out(LOG_DEBUG,  "new mouse pos  : [%04x, %04x]", absMouse.x, absMouse.y);
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_MOUSE_BTN_ACTION - param: %02X", (int) bfr[1]);
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_Y_AT_TOP");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_SET_Y_AT_BOTTOM");
            ...
            Debug::out(LOG_DEBUG,  "Ikbd::processStCommands -- ST says: STCMD_DISABLE_MOUSE");
...
void Ikbd::processKeyboardData(bool skipKeyboardTranslation)
{
    ...
    while(cbKeyboardData.count > 0) {                           // while there are some data, process
        ...
        Debug::out(LOG_DEBUG,  "Ikbd::processKeyboardData -- got %02x, cb contains %d bytes", val, cbKeyboardData.count);
        ...
        if(val >= KEYBDATA_SPECIAL_LOWEST && val <= 0xff) {     // if this a special code?
			...
			switch(stKeyboadCmd) {                              // handle data sent from original keyboard
                ...
                Debug::out(LOG_DEBUG,  "Ikbd::processKeyboardData - keyboard says: KEYBDATA_MOUSE_ABS");

				if(gotUsbMouse()) {								// ...and we got USB mouse? Don't resend.
                    Debug::out(LOG_DEBUG,  "Ikbd::processKeyboardData - KEYBDATA_MOUSE_ABS -- got USB mouse, won't resend ABS pos from ST");
					resendTheseData = false;
				} else {
                    Debug::out(LOG_DEBUG,  "Ikbd::processKeyboardData - KEYBDATA_MOUSE_ABS -- no USB mouse, will resend ABS pos from ST");
                }
                ...
                if (firstJoyIs0 && joystickState == Enabled) {   // don't translate mouse in port0 as port1
                    ...
                    Debug::out(LOG_DEBUG,  "Ikbd::processKeyboardData - joy0->joy1 but not joystick mode is off, leaving as joy0");
                } else {
                    Debug::out(LOG_DEBUG, "firstJoy0: %d (joy0)", firstJoyIs0);
                }
                break;
                ...
                if (firstJoyIs0 && joystickState == Enabled) {   // don't map joy1 if joy0 is used as mouse
                    ...
                    Debug::out(LOG_DEBUG,  "Ikbd::processKeyboardData - joy1->joy0 but not joystick mode is off, leaving as joy1");
                } else {
                    Debug::out(LOG_DEBUG, "firstJoy0: %d (joy1)", firstJoyIs0);
                }
                break;

				//----------------------------------------------
				case KEYBDATA_JOY_BOTH:							// ST asked for both joystick states (interrogation) and this is the response
                Debug::out(LOG_DEBUG,  "Ikbd::processKeyboardData - keyboard says: KEYBDATA_JOY_BOTH");
