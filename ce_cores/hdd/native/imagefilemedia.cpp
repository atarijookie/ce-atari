...
bool ImageFileMedia::iopen(const char *path, bool createIfNotExists)
{
    ...
    if(fd == -1) {
        logHdd(LOG_ERROR, "ImageFileMedia - failed to open %s : %s", path, strerror(errno));
        return false;
    }
    ...
    if(imageWasCreated) {               // if the image was just created, create empty image file
        ...
        logHdd(LOG_DEBUG, "ImageFileMedia - creating %s, filling with 1MB of 0",path);
    ...
    struct stat st;
    if(fstat(fd, &st) < 0) {
        logHdd(LOG_ERROR, "fstat(%d) %s FAILED : %s", fd, path, strerror(errno));
    } else {
    ...
    logHdd(LOG_DEBUG, "ImageFileMedia - open succeeded, capacity: %lld sectors / %lld bytes, was created: %d",
...
bool ImageFileMedia::readSectors(int64_t sectorNo, uint32_t count, uint8_t *bfr)
{
    ...
    if(sectorNo >= SCapacity) {
        logHdd(LOG_ERROR, "ImageFileMedia::readSectors(%lld) after end of disk : total # of sectors = %lld",
    ...
    if(ofs != pos) {                              // failed to seek?
        logHdd(LOG_ERROR, "ImageFileMedia - lseek failed %lld %s", (long long)ofs, strerror(errno));
        return false;
    }
    ...
    while(byteCount > 0) {
        ...
        if(res < 0) {
            logHdd(LOG_ERROR, "ImageFileMedia - read error : %s", strerror(errno));
            return false;
        } else if(res == 0) {
            logHdd(LOG_ERROR, "ImageFileMedia - read returned 0 (byteCount=%llu)", (long long unsigned)byteCount);
            return false;
        }
        byteCount -= res;
        bfr += res;
...
bool ImageFileMedia::writeSectors(int64_t sectorNo, uint32_t count, uint8_t *bfr)
{
    ...
    if(sectorNo >= SCapacity) {
        logHdd(LOG_ERROR, "ImageFileMedia::writeSectors(%lld) after end of disk : total # of sectors = %lld",
                   (long long)sectorNo, (long long)SCapacity);
        return false;
    }
    ...
    if(ofs != pos) {                              // failed to seek?
        logHdd(LOG_ERROR, "ImageFileMedia - lseek failed %lld %s", ofs, strerror(errno));
        return false;
    }
    ...
    while(byteCount > 0) {
        ...
        if(res < 0) {
            logHdd(LOG_ERROR, "ImageFileMedia - write error : %s", strerror(errno));
            return false;
        } else if(res == 0) {
            logHdd(LOG_ERROR, "ImageFileMedia - write returned 0");
            return false;
        }
        byteCount -= res;
        bfr += res;
