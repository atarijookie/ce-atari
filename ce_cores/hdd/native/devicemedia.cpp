...
bool DeviceMedia::iopen(const char *path, bool createIfNotExists)
{
    ...
    logHdd(LOG_DEBUG, "DeviceMedia - open succeeded, capacity: %d MB, sectors: %08x", (int) capacityInMB, SCapacity);
...
bool DeviceMedia::readSectors(int64_t sectorNo, uint32_t count, uint8_t *bfr)
{
    if(!isInit()) {                             // if not initialized, failed
        logHdd(LOG_DEBUG, "DeviceMedia::readSectors - not init");
        return false;
    }
	...
	if(ofs != pos) {
		logHdd(LOG_ERROR, "DeviceMedia::readSectors - lseek failed %lld", (long long)ofs, strerror(errno));
		return false;
	}
#else
    ...
    if(res < 0) {                              	// failed to lseek?
        logHdd(LOG_DEBUG, "DeviceMedia::readSectors - lseek64() failed, errno: %d", errno);
        return false;
    }
#endif
    ...
    if(cnt != byteCount) {                      // not all data was read? fail
        logHdd(LOG_DEBUG, "DeviceMedia::readSectors - read() failed");
        return false;
    }
...
{
    if(!isInit()) {                             // if not initialized, failed
        logHdd(LOG_DEBUG, "DeviceMedia::writeSectors - not init");
        return false;
    }
	...
	off_t ofs = lseek(fdes, pos, SEEK_SET);
	if(ofs != pos) {
		logHdd(LOG_ERROR, "DeviceMedia::writeSectors - lseek failed %lld", (long long)ofs, strerror(errno));
		return false;
	}
#else
    off64_t pos = sectorNo * ((int64_t)512);    // convert sector # to offset
    ...
    if(res < 0) {                              	// failed to lseek?
        logHdd(LOG_DEBUG, "DeviceMedia::writeSectors - lseek64() failed, errno: %d", errno);
        return false;
    }
#endif
    ...
    if(cnt != byteCount) {                      // not all data was writen? fail
        logHdd(LOG_DEBUG, "DeviceMedia::writeSectors - write() failed");
        return false;
    }
