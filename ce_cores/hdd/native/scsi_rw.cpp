...
void Scsi::readWriteGeneric(bool readNotWrite, uint32_t startingSector, uint32_t sectorCount)
{
    ...
    if(totalByteCount > 0xffffff) {                                     // if trying to send more than 16 MB of data at once, fail
        logHdd(LOG_ERROR, "Scsi::readWriteGeneric() - tried to transfer more than 16 MB of data of once, fail");
    ...
    if((startingSector + sectorCount - 1) > mediaCapacityInSectors) {   // trying to read out of range?
        logHdd(LOG_ERROR, "Scsi::readWriteGeneric() - tried to go out of range");
...
bool Scsi::readSectors_small(uint32_t startSectorNo, uint32_t sectorCount)
{
    ...
    logHdd(LOG_DEBUG, "Scsi::readSectors_small() - startSectorNo: 0x%x, sectorCount: 0x%x", startSectorNo, sectorCount);

    res = dataMedia->readSectors(startSectorNo, sectorCount, dataBuffer);

    if(!res) {
        logHdd(LOG_ERROR, "Scsi::readSectors_small() - failed for startSectorNo: 0x%x, sectorCountNow: 0x%x", startSectorNo, sectorCount);
        return false;
    }

    dataTrans->addDataBfr(dataBuffer, totalByteCount, true);

    logHdd(LOG_DEBUG, "Scsi::readSectors_small() - done with success");
    return true;
}
...
bool Scsi::readSectors_big(uint32_t startSectorNo, uint32_t sectorCount)
{
    ...
    logHdd(LOG_DEBUG, "Scsi::readSectors() - startSectorNo: 0x%x, sectorCount: 0x%x -- will do %d loops", startSectorNo, sectorCount, (sectorCount / BUFFER_SIZE_SECTORS) + 1);
    ...
    if(!res) {
        logHdd(LOG_ERROR, "Scsi::readSectors() - sendData_start() failed");
        return false;
    }
    ...
    while(sectorCount > 0) {
        ...
        logHdd(LOG_DEBUG, "Scsi::readSectors() - will read sectorCountNow: 0x%x, sectors to go: 0x%x", sectorCountNow, sectorCount - sectorCountNow);

        // read sector from media
        res = dataMedia->readSectors(startSectorNo, sectorCountNow, dataBuffer);

        if(!res) {
            logHdd(LOG_ERROR, "Scsi::readSectors() - dataMedia->readSectors() failed for startSectorNo: 0x%x, sectorCountNow: 0x%x", startSectorNo, sectorCountNow);
            return false;
        }
        ...
        if(!res) {
            logHdd(LOG_ERROR, "Scsi::readSectors() - dataTrans->sendData_transferBlock() failed for startSectorNo: 0x%x, sectorCountNow: 0x%x", startSectorNo, sectorCountNow);
            return false;
        }
    }

    logHdd(LOG_DEBUG, "Scsi::readSectors() - done with success");
    return true;
}
...
bool Scsi::writeSectors_small(uint32_t startSectorNo, uint32_t sectorCount)
{
    ...
    logHdd(LOG_DEBUG, "Scsi::writeSectors_small() - startSectorNo: 0x%x, sectorCount: 0x%x", startSectorNo, sectorCount);

    uint32_t totalByteCount = sectorCount * 512;
    res = dataTrans->recvData(dataBuffer, totalByteCount);

    if(!res) {
        logHdd(LOG_ERROR, "Scsi::writeSectors() - recvData() failed");
        return false;
    }
    ...
    if(!res) {
        logHdd(LOG_ERROR, "Scsi::writeSectors() - dataMedia->writeSectors() failed");
        return false;
    }

    logHdd(LOG_DEBUG, "Scsi::writeSectors() - done with success");
    return true;
}
...
bool Scsi::writeSectors_big(uint32_t startSectorNo, uint32_t sectorCount)
{
    ...
    logHdd(LOG_DEBUG, "Scsi::writeSectors() - startSectorNo: 0x%x, sectorCount: 0x%x -- will do %d loops", startSectorNo, sectorCount, (sectorCount / BUFFER_SIZE_SECTORS) + 1);
    ...
    if(!res) {
        logHdd(LOG_ERROR, "Scsi::writeSectors() - recvData_start() failed");
        return false;
    }
    ...
    while(sectorCount > 0) {
        ...
        logHdd(LOG_DEBUG, "Scsi::writeSectors() - will write sectorCountNow: 0x%x, sectors to go: 0x%x", sectorCountNow, sectorCount - sectorCountNow);
        ...
        if(!res) {
            logHdd(LOG_ERROR, "Scsi::writeSectors() - dataTrans->recvData_transferBlock() failed");
            return false;
        }
...
bool Scsi::compareSectors(uint32_t startSectorNo, uint32_t sectorCount)
{
    ...
    logHdd(LOG_DEBUG, "Scsi::compareSectors() - startSectorNo: %d, sectorCount: %d -- will do %d loops", startSectorNo, sectorCount, (sectorCount / BUFFER_SIZE_SECTORS) + 1);

    uint32_t totalByteCount = sectorCount * 512;
    res = dataTrans->recvData_start(totalByteCount);        // try to start the write data transfer

    if(!res) {
        logHdd(LOG_ERROR, "Scsi::compareSectors() - recvData_start() failed");
        return false;
    }
    ...
    while(sectorCount > 0) {
        ...
        if(!res) {
            logHdd(LOG_ERROR, "Scsi::compareSectors() - dataTrans->recvData_transferBlock() failed");
            return false;
        }
        ...
        if(!res) {
            logHdd(LOG_ERROR, "Scsi::compareSectors() - dataMedia->readSectors() failed");
            return false;
        }
        ...
        if(iRes != 0) {                                             // data is different?
            logHdd(LOG_ERROR, "Scsi::compareSectors() - data mismatch");
            return false;
        }
