...
void TranslatedDisk::findAttachedDisks(void)
{
    ...
    for(int i=2; i<MAX_DRIVES; i++) {               // go through all the possible drives
        ...
        if(configDriveIndex == i) {
            ...
            logHdd(LOG_DEBUG, "TranslatedDisk::findAttachedDisks: [%d] %c: -> %s", i, driveLetter, conf[i].hostRootPath.c_str());
            continue;
        }
...
{
    if(dataTrans == 0 ) {
        logHdd(LOG_ERROR, "processCommand was called without valid dataTrans!");
        return;
    }
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk function - %s (%02x)", functionName, cmd[4]);
...
void TranslatedDisk::onInitialize(void)     // this method is called on the startup of CosmosEx translated disk driver
{
    ...
    if(!res) {                                      // failed to get data? internal error!
        logHdd(LOG_ERROR, "TranslatedDisk::onInitialize - failed to receive data...");
    ...
    logHdd(LOG_DEBUG, "tosVersion: %x, drives: %04x", tosVersion, drives);
...
void TranslatedDisk::onGetConfig(uint8_t *cmd)
{
    ...
    dataTrans->addDataWord(year);                   // bytes 7, 8 - year
    dataTrans->addDataByte(month);                  // byte 9     - month
    dataTrans->addDataByte(day);                    // byte 10    - day

    dataTrans->addDataByte(hour);                   // byte 11 - hours
    dataTrans->addDataByte(minute);                 // byte 12 - minutes
    dataTrans->addDataByte(second);                 // byte 13 - seconds

    logHdd(LOG_DEBUG, "onGetConfig - setDateTime: %d, utcOffset: %.1f", (int) setDateTime, (float) utcOffset);
    logHdd(LOG_DEBUG, "onGetConfig - date: %04d-%02d-%02d, time: %02d:%02d:%02d", year, month, day, hour, minute, second);
    //------------------
    ...
    //-----------------
    logHdd(LOG_DEBUG, "onGetConfig - frameSkip: %d, TRANSLATEDDISK_VERSION: 0x%04X, screenShotVblEnabled: %d, doScreenShot: %d",
        frameSkip, TRANSLATEDDISK_VERSION, (int) events.screenShotVblEnabled, (int) events.doScreenShot);
...
bool TranslatedDisk::hostPathExists(std::string &hostPath, bool alsoCheckCaseInsensitive)
{
    ...
    if(res != -1) {             // if it's not this error, then the file exists
        logHdd(LOG_DEBUG, "TranslatedDisk::hostPathExists( %s ) == TRUE (file / dir exists)", hostPath.c_str());
        return true;
    }
    ...
    logHdd(LOG_DEBUG, "TranslatedDisk::hostPathExists( %s ) == FALSE (file / dir does not exist)", hostPath.c_str());
    return false;
}
...
bool TranslatedDisk::hostPathExists_caseInsensitive(std::string hostPath, std::string &justPath, std::string &originalFileName, std::string &foundFileName)
{
    ...
    while(1) {                                                      // while there are more files, store them
        ...
        if(inFileUpperCase.compare(foundFile) == 0) {   // in input upper case file matches this found file
            logHdd(LOG_DEBUG, "TranslatedDisk::hostPathExists_caseInsensitive -- found that input file %s matches file %s", originalFileName.c_str(), de->d_name);
...
bool TranslatedDisk::createFullAtariPath(std::string inPartialAtariPath, std::string &outFullAtariPath, int &outAtariDriveIndex)
{
    ...
    //logHdd(LOG_DEBUG, "TranslatedDisk::createFullAtariPath - inPartialAtariPath: %s", inPartialAtariPath.c_str());
    ...
    if(inPartialAtariPath[1] == ':') {
        ...
        if(outAtariDriveIndex == -1) {
            logHdd(LOG_DEBUG, "TranslatedDisk::createFullAtariPath - invalid drive letter '%c' -- %s", inPartialAtariPath[0], inPartialAtariPath.c_str());
            return false;
        }
        ...
        logHdd(LOG_DEBUG, "TranslatedDisk::createFullAtariPath - with drive letter, absolute path : %s, drive index: %d => %s", inPartialAtariPath.c_str(), outAtariDriveIndex, outFullAtariPath.c_str());
        return true;
    }
    ...
    if(!conf[currentDriveIndex].enabled) {                          // we're trying this on disabled drive?
        logHdd(LOG_DEBUG, "TranslatedDisk::createFullAtariPath - the current drive %d is not enabled (not translated drive)", currentDriveIndex);
        return false;
    }

    if(Utils::startsWith(inPartialAtariPath, HOSTPATH_SEPAR_STRING)) {         // starts with backslash? absolute path on current drive
        outFullAtariPath = inPartialAtariPath.substr(1);
        removeDoubleDots(outFullAtariPath);                             // search for '..' and simplify the path

        logHdd(LOG_DEBUG, "TranslatedDisk::createFullAtariPath - without drive letter, absolute path : %s => %s ", inPartialAtariPath.c_str(), outFullAtariPath.c_str());
        return true;
        ...
        logHdd(LOG_DEBUG, "TranslatedDisk::createFullAtariPath - without drive letter, relative path : %s => %s", inPartialAtariPath.c_str(), outFullAtariPath.c_str());
        return true;
    }
}
...
int TranslatedDisk::driveLetterToDriveIndex(char pathDriveLetter)
{
    ...
    if(!isValidDriveLetter(pathDriveLetter)) {          // not a valid drive letter?
        logHdd(LOG_DEBUG, "TranslatedDisk::createHostPath - invalid drive letter");
        return -1;
    }
    ...
    if(driveIndex < 2) {                                // drive A and B not handled
        logHdd(LOG_DEBUG, "TranslatedDisk::createHostPath - drive A & B not handled");
        return -1;
    }

    if(!conf[driveIndex].enabled) {                     // that drive is not enabled?
        logHdd(LOG_DEBUG, "TranslatedDisk::createHostPath - drive not enabled");
        return -1;
    }
...
//    logHdd(LOG_DEBUG, "removeDoubleDots before: %s", path.c_str());
    ...
    while(1) {
        ...
        if(found >= MAX_DIR_NESTING) {              // sanitize possible overflow
            logHdd(LOG_ERROR, "removeDoubleDots has reached maximum dir nesting level, not removing double dost!");
            break;
        }
    }
...
//    logHdd(LOG_DEBUG, "removeDoubleDots after: %s", final.c_str());
    path = final;
}
...
int TranslatedDisk::deleteDirectoryPlain(const char *path)
{
    ...
    logHdd(LOG_ERROR, "TranslatedDisk::deleteDirectoryPlain rmdir(%s) : %s", path, strerror(errno));
    return EACCDN;
}
...
bool TranslatedDisk::isRootDir(std::string hostPath)
{
    ...
    for(int i=2; i<MAX_DRIVES; i++) {                   // go through all translated drives
        ...
        //logHdd(LOG_DEBUG, "TranslatedDisk::isRootDir - %s == %s ?", conf[i].hostRootPath.c_str(), hostPath.c_str());
        if(conf[i].hostRootPath == hostPath) {          // ok, this is root dir!
            logHdd(LOG_DEBUG, "TranslatedDisk::isRootDir - hostPath: %s -- yes, it's a root dir", hostPath.c_str());
            return true;
        }
    }

    logHdd(LOG_DEBUG, "TranslatedDisk::isRootDir - hostPath: %s -- no, it's NOT a root dir", hostPath.c_str());
...
void TranslatedDisk::onStLog(uint8_t *cmd)
{
    ...
    if(!res) {                                      // failed to get data? internal error!
        logHdd(LOG_ERROR, "TranslatedDisk::onStLog - failed to receive data...");
        dataTrans->setStatus(EINTRN);
        return;
    }

    logHdd(LOG_DEBUG, "ST log: %s", dataBuffer);
    dataTrans->setStatus(E_OK);
}
...
void TranslatedDisk::onStHttp(uint8_t *cmd)
{
    ...
    if(!res) {                                      // failed to get data? internal error!
        logHdd(LOG_ERROR, "TranslatedDisk::onStHttp - failed to receive data...");
        dataTrans->setStatus(EINTRN);
        return;
    }
...
//    Downloader::add(tdr);
//    logHdd(LOG_DEBUG, "TranslatedDisk::onStHttp() %s", dataBuffer);

    logHdd(LOG_DEBUG, "ST HTTP: %s", dataBuffer);
    dataTrans->setStatus(E_OK);
}
