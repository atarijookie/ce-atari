...
{
    if(!com) {
        logHdd(LOG_ERROR, "AcsiDataTrans::sendDataAndStatus -- no communication object, fail!");
        return;
    }
    ...
    //---------------------------------------
    if(dumpNextData) {
        logHdd(LOG_DEBUG, "sendDataAndStatus: %d bytes", count);
        Debug::outBfr(LOGFILE_HDD, buffer, count);
        dumpNextData = false;
    }
    //---------------------------------------
...
{
    if(!com) {
        logHdd(LOG_ERROR, "AcsiDataTrans::sendData_start -- no communication object, fail");
        return false;
    }
...
{
    if(!com) {
        logHdd(LOG_ERROR, "AcsiDataTrans::recvData_start() -- no communication object, fail!");
        return false;
    }
