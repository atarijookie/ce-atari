...
{
    uint8_t buf[2] = {0, cmd};
    int res = i2c_write_timeout_us(DISPLAY_I2C_IFACE, SSD1306_I2C_ADDRESS, buf, 2, false, 100000);
    return (res >= 0);  // okay if result not negative
}
...
void SSD1306::display(void)
{
    ...
    uint8_t bfr[SSD1306_BYTES_PER_LINE + 1];
    ...
    for (y=0; y<SSD1306_LCDHEIGHT; y++) {
        ...
        bfr[0] = 0x40;
        memcpy(bfr + 1, data, SSD1306_BYTES_PER_LINE);

        i2c_write_timeout_us(DISPLAY_I2C_IFACE, SSD1306_I2C_ADDRESS, bfr, SSD1306_BYTES_PER_LINE + 1, false, 100000);
    }
}
