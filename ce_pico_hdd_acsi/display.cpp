...
void displayInit(void)
{
    ...
    gpio_set_function(PIN_DS, GPIO_FUNC_SIO);
    gpio_set_function(PIN_CP, GPIO_FUNC_SIO);

    gpio_set_dir_out_masked((1 << PIN_DS) | (1 << PIN_CP));
...
void display(uint8_t what)
{
    ...
    for(int i = 7; i >= 0; i--) {
        ...
        BIT_CLR(PIN_CP);    // CP to L
        busy_wait_at_least_cycles(10);
        BIT_SET(PIN_CP);    // CP to H
        busy_wait_at_least_cycles(10);
        BIT_SET(PIN_CP);    // CP to L
    }

    // in GET_COMMAND mode (selection) the display pins are used as SIO inputs
    gpio_set_dir_in_masked((1 << PIN_DS) | (1 << PIN_CP));
}
