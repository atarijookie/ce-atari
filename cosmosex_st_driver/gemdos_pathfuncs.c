...
int32_t custom_dgetdrv( void *sp )
{
	...
	if(!isOurDrive(getSetCurrentDrive(GET_CURRENTDRIVE), 0)) {									/* if the current drive is not our drive */
		CALL_OLD_GD_NORET(Dgetdrv);
	
		getSetCurrentDrive(res);												/* store the current drive */
		return res;
	}
...
int32_t custom_dsetdrv( void *sp )
{
	...
	getSetCurrentDrive(drive);												/* store the drive - GEMDOS seems to let you set even invalid drive */
	...
	if(!isOurDrive(drive, 0)) {											/* if the drive is not our drive */
        ...
        acsi_cmd(ACSI_WRITE, commandShort, CMD_LENGTH_SHORT, pDmaBuff, 1);	/* send command to host over ACSI */

        CALL_OLD_GD_NORET(Dsetdrv, drive);
	
		res = res | getSetCeDrives(GET_CEDRIVES);							/* mounted drives = original drives + ceDrives */	
		return res;
	}
...
int32_t custom_dfree( void *sp )
{
	...
	memcpy(pDiskInfo, pDmaBuff, 16);									/* copy in the results */
...
int32_t custom_dcreate( void *sp )
{
	...
	BYTE *pDmaBuff = getDmaBufferPointer();
	memset(pDmaBuff, 0, 512);
	strncpy((char *) pDmaBuff, (char *) pPath, DMA_BUFFER_SIZE);		/* copy in the path */
...
int32_t custom_dgetpath( void *sp )
{
	...
	strncpy((char *)buffer, (char *)pDmaBuff, DMA_BUFFER_SIZE);		/* copy in the results */
    return extendByteToDword(res);
}
...
int32_t custom_frename( void *sp )
{
	...
	BYTE *pDmaBuff = getDmaBufferPointer();
	memset(pDmaBuff, 0, DMA_BUFFER_SIZE);
	strncpy((char *) pDmaBuff, oldName, (DMA_BUFFER_SIZE / 2) - 2);	/* copy in the old name	*/
	
	int oldLen = strlen((char *) pDmaBuff);							/* get the length of old name */
	
	char *pDmaNewName = ((char *) pDmaBuff) + oldLen + 1;
...
int32_t custom_fattrib( void *sp )
{
	...
	pDmaBuff[0] = (BYTE) flag;										/* store set / get flag */
	pDmaBuff[1] = (BYTE) attr;										/* store attributes */
	
	strncpy(((char *) pDmaBuff) + 2, fileName, DMA_BUFFER_SIZE -1 );	/* copy in the file name */
